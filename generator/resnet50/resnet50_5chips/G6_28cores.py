...
def gen_g6_map_config(phase_en, clock_in_phase, size_x, size_y, chip, cuts, static_data=None,
        ...
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            if out_data_en:
                soma1 = p06(addr_in=0x1c800 >> 2, addr_out=0x24000 >> 2, addr_ciso=0x0000 >> 2, length_in=512,
                            ...
                            data_in=static_data['g6_add_output1'][(core_x, core_y)] if init_data else None)
            else:
                soma1 = None
        ...
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            if out_data_en:
                soma1 = p06(addr_in=0x1e400 >> 2, addr_out=0x24000 >> 2, addr_ciso=0x0000 >> 2, length_in=512,
                            num_in=14, length_ciso=1, num_ciso=14, length_out=512, num_out=14, type_in=1, type_out=1,
                            data_in=static_data['g6_add_output2'][(core_x, core_y)] if init_data else None)
            else:
                soma1 = None
            addr_rhead_base = MapConfigGen.get_router_rhead_base(phase_group[(chip, (core_x, core_y))]['prims'])
            router = p09(rhead_mode=1, send_en=out_data_en, receive_en=in_data_en, send_num=895, receive_num=3,
    ...
    if phase_en[offset + 0]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            if static_data is None:
                ...
                data_x = None if in_data_en else static_data['conv3b1_input'][(core_x, core_y)]
                data_w = static_data['conv3b1_weight'][(core_x, core_y)]
                data_b = static_data['conv3b1_bias'][(core_x, core_y)]
            ...
            if core_x < 7 and core_y == 0:
                soma1 = pX5(mode='max', addr_in=0x1C800 >> 2, addr_out=0x8400 >> 2, cin=32, cout=32, px=28, py=1, kx=1,
                            ky=1, sx=1, sy=1, cmp_c=0, type_in=0, type_out=1, in_cut_start=cuts['layer2.1.cut1'],
    ...
    if phase_en[offset + 17]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            if core_x % 7 == 0:
                ...
                py = 6
                addr_ina = 0x9580 >> 2
            data_w = static_data['conv3b2_weight'][(core_x, core_y)]
            data_b = static_data['conv3b2_bias'][(core_x, core_y)]
            ...
            soma1 = pX5(mode='max', addr_in=0x1C800 >> 2, addr_out=0x1E400 >> 2, cin=32, cout=32, px=28, py=4, kx=1,
                        ky=1, sx=1, sy=1, cmp_c=0, type_in=0, type_out=1, in_cut_start=cuts['layer2.1.cut2'],
    ...
    if phase_en[offset + 20]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            data_w = static_data['conv3b3_weight'][(core_x, core_y)]
            data_b = static_data['conv3b3_bias'][(core_x, core_y)]
            ...
            soma1 = pX5(mode='max', addr_in=0x8400 >> 2, addr_out=0xBC00 >> 2, cin=128, cout=128, px=28, py=4, kx=1,
                        ky=1, sx=1, sy=1, cmp_c=0x80808080, type_in=0, type_out=1, in_cut_start=cuts['layer2.1.cut3'],
                        row_ck_on=1, in_row_max=1)
            router = None
            soma2 = None
            phase_group[(chip, (core_x, core_y))]['prims'].append({'axon': axon, 'soma1': soma1, 'router': router,
    ...
    if phase_en[offset + 23]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            soma1 = pX5(mode='max', addr_in=0x8400 >> 2, addr_out=0x1c800 >> 2, cin=512, cout=512, px=1, py=28,
                        kx=1, ky=1, sx=1, sy=1, cmp_c=0, type_in=0, type_out=1, in_cut_start=cuts['layer2.1.cut5'],
                        row_ck_on=1, in_row_max=2)
            router = None
            soma2 = None
            phase_group[(chip, (core_x, core_y))]['prims'].append({'axon': axon, 'soma1': soma1, 'router': router,
...
if __name__ == '__main__':
    ...
    static_data = generate_g6_data(handler, size_y=2, size_x=14)
    config = gen_g6_map_config(phase, clock_in_phase=100_000, size_x=14, size_y=2, chip=(0, 0), cuts=cuts,
