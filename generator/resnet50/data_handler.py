...
class ResNetDataHandler(DataHandler):
    ...
    def __init__(self, seed=1):
        ...
        self.__model = resnet50(pretrained=False, progress=False, batch_norm=False, quantization=True)
        # ckpt_path = './generator/resnet50/checkpoints/cifar100_resnet50_quantized_78.920.pth'
        ckpt_path = './generator/resnet50/checkpoints/resnet50_imagenet_top1_69.757996_quantized.pth'
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        pretrained_dict = torch.load(ckpt_path, map_location=device)
        self.__model.load_state_dict(pretrained_dict['state_dict'])
