...
def gen_g17_map_config(phase_en, clock_in_phase, size_x, size_y, cuts, data=None,
                       in_data_en=False, out_data_en=False, delay_l4=None, delay_l5=None, chip=(0, 0), init_data=None):
    """
        ResNet-50 5-Chip Group-17
        core array : 4 * 16
    """
    map_config = {
        'sim_clock': None,
    ...
    # 接收 24*512/1024=12.5   **  发送1*1*128
    if phase_en[0]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            axon = p41(px=1, py=1, load_bias=0, cin=1, cout=1, kx=1, ky=1, sx=1, sy=1, addr_ina=0xc000 >> 2,
                       addr_inb=0xc000 >> 2, addr_bias=0xc000 >> 2, addr_out=0xc000 >> 2, axon_delay=True,
            ...
            if out_data_en and core_y == 0:
                soma1 = pX5(mode='max', addr_in=0x1f200 >> 2, addr_out=0x9000, cin=128, cout=128, px=1, py=1, kx=1,
                            ky=1, sx=1, sy=1, cmp_c=0x80808080, type_in=1, type_out=1, in_cut_start=0, row_ck_on=0,
                            in_row_max=1, data_in=data['avgpool']['output'][(core_x, core_y)] if init_data else None)
            else:
                soma1 = None
            addr_rhead_base = MapConfigGen.get_router_rhead_base(phase_group[(chip, (core_x, core_y))]['prims'],
                                                                 limit=0x380)
            router = p09(rhead_mode=1, send_en=out_data_en and core_y == 0, receive_en=in_data_en, send_num=15,
                         receive_num=7,
                         ...
                         cxy=0, relay_num=1535, nx=0, ny=0, data_in=None)
            if out_data_en and core_y == 0:
                router.addRHead(S=0, T=1, P=0, Q=0, X=-1 - core_x, Y=1 - core_y, A=core_x * 16 + 16 * 128 * 3 // 8,
                                pack_per_Rhead=15, A_offset=0, Const=0, EN=1)
            if in_data_en:
                if core_y == 0:
                    if core_x == 15:
                    ...
                    if core_x == 15:
                        router.CXY, router.Nx, router.Ny = 0, 0, 0
                    elif core_x in [1, 2, 3, 4]:
                        router.CXY, router.Nx, router.Ny = 1, -1, 0
                    elif core_x == 0:
                        router.CXY, router.Nx, router.Ny = 1, 5, 0
                    else:
                        router.CXY, router.Nx, router.Ny = 1, 1, 0
                elif core_y == 2:
                    if core_x == 15:
                        router.CXY, router.Nx, router.Ny = 0, 0, 0
                    elif core_x in [1, 2, 3, 4, 5, 6, 7, 8]:
                        router.CXY, router.Nx, router.Ny = 1, -1, 0
                    elif core_x == 0:
                        router.CXY, router.Nx, router.Ny = 1, 9, 0
                    else:
                        router.CXY, router.Nx, router.Ny = 1, 1, 0
                elif core_y == 3:
                    if core_x == 0:
                        router.CXY, router.Nx, router.Ny = 0, 0, 0
                    elif core_x in [12, 13, 14]:
                        router.CXY, router.Nx, router.Ny = 1, 1, 0
                    elif core_x == 15:
                    ...
                    raise ValueError
            soma2 = p06(addr_in=0x8380, addr_out=0x19000 >> 2, addr_ciso=0, length_in=512, length_out=512,
    ...
    if phase_en[1]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            addr_rhead_base = MapConfigGen.get_router_rhead_base(phase_group[(chip, (core_x, core_y))]['prims'],
                                                                 limit=0x380)
            router = p09(rhead_mode=1, send_en=0, receive_en=in_data_en, send_num=0,
                         receive_num=7,
            ...
            if in_data_en:
                if core_y == 0:
                    ...
                    raise ValueError
            soma2 = p06(addr_in=0x8380, addr_out=0x1c000 >> 2, addr_ciso=0, length_in=512, length_out=512,
    ...
    if phase_en[offset + 0]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            axon = p41(px=7, py=7, cin=512, cout=32, kx=1, ky=1, sx=1, sy=1, addr_ina=0x19000 >> 2,
                       addr_inb=0x300 >> 2, addr_bias=0x0 >> 2, addr_out=0x8300 >> 2, ina_type=1, inb_type=1,
                       load_bias=2, pad_top=0, pad_down=0, pad_left=0, pad_right=0,
                       data_x=data['layer4.2.conv1']['input'][(core_x, core_y)] if (not in_data_en) else None,
                       data_w=data['layer4.2.conv1']['weight'][(core_x, core_y)],
                       data_b=data['layer4.2.conv1']['bias'][(core_x, core_y)])
            soma1 = pX5(mode='max', addr_in=0x8300 >> 2, addr_out=0x9000, cin=32, cout=32, px=7, py=7, kx=1,
            ...
            soma2 = p06(addr_in=0x8380, addr_out=0x8300 >> 2, addr_ciso=0, length_in=length, length_out=length,
                        length_ciso=1, num_in=4, num_ciso=4, num_out=4, type_in=0, type_out=0,
                        in_cut_start=0, in_row_max=0, row_ck_on=0, data_in=None, data_ciso=None)
            phase_group[(chip, (core_x, core_y))]['prims'].append({'axon': axon, 'soma1': soma1, 'router': router,
                                                                   'soma2': soma2})

    # L44 部分和求和，流水ReLU，发送然后整理 - 发送
    if phase_en[offset + 1]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            axon = p02(avg_pooling_en=False, ina_type=0, load_bias=0, kx=1, ky=4, sx=1, sy=1, cin=32, px=px, py=py,
                       addr_in=0x8300 >> 2, addr_bias=0x0, addr_out=0x1f200 >> 2, pad_top=0, pad_down=0, pad_left=0,
                       pad_right=0, bias_length=0, data_x=None, data_b=None, constant_b=0)
            soma1 = None
            router = None
            soma2 = pX5(mode='max', addr_in=0x1f200 >> 2, addr_out=0x8300 >> 2, cin=32, cout=32, px=px, py=py, kx=1,
                        ky=1, sx=1, sy=1, cmp_c=0x0, type_in=0, type_out=1, in_cut_start=cuts['layer4.2.cut1'],
    ...
    if phase_en[offset + 2]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            if core_y in [0]:
                ...
                px, py = 12, 1
            axon = None
            soma1 = pX5(mode='max', addr_in=0x8300 >> 2, addr_out=0x9000, cin=32, cout=32, px=px, py=py, kx=1,
    ...
    if phase_en[offset + 3]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            soma2 = p06(addr_in=0x8380, addr_out=0x8300 >> 2, addr_ciso=0x0000, length_in=7 * 128,
                        length_out=7 * 128,
    ...
    if phase_en[offset + 4]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            axon = None
            # 先从512通道中抽取128通道
            addr_in = (0x19000 + 128 * (core_x % 4)) >> 2
            soma1 = p06(addr_in=addr_in, addr_out=0x9000, addr_ciso=0, length_in=512, length_out=128,
                        length_ciso=1, num_in=49, num_ciso=49, num_out=49, type_in=1, type_out=1,
                        in_cut_start=0, in_row_max=0, row_ck_on=0, data_in=None, data_ciso=None)
            # 然后发送给一列 12, 12, 12, 13
            if core_y in [3]:
                addr_din_length = 207  # 13*128//8-1
                length = 13 * 128
            else:
                addr_din_length = 191  # 12*128//8-1
                length = 12 * 128
            addr_rhead_base = MapConfigGen.get_router_rhead_base(phase_group[(chip, (core_x, core_y))]['prims'],
                                                                 limit=0x380)
            router = p09(rhead_mode=1, send_en=0, receive_en=1, send_num=783, receive_num=0, addr_din_base=0x380,
                         addr_din_length=addr_din_length, addr_rhead_base=addr_rhead_base, addr_rhead_length=1,
                         addr_dout_base=0x1000, addr_dout_length=0, soma_in_en=1, cxy=0, relay_num=0, data_in=None)
            if (core_x, core_y) in [(0, 0), (1, 0), (2, 0), (3, 0), (4, 1), (5, 1), (6, 1), (7, 1), (8, 2), (9, 2),
                                    (10, 2), (11, 2), (12, 3), (13, 3), (14, 3), (15, 3)]:
                router.Send_en = 1
                router.addRHead(S=0, T=1, P=0, Q=0, X=0, Y=0 - core_y, A=0, pack_per_Rhead=191, A_offset=0, Const=0,
                                EN=1)
                router.addRHead(S=0, T=1, P=0, Q=0, X=0, Y=1 - core_y, A=0, pack_per_Rhead=191, A_offset=0, Const=0,
                                EN=1)
                router.addRHead(S=0, T=1, P=0, Q=0, X=0, Y=2 - core_y, A=0, pack_per_Rhead=191, A_offset=0, Const=0,
                                EN=1)
                router.addRHead(S=0, T=1, P=0, Q=0, X=0, Y=3 - core_y, A=0, pack_per_Rhead=207, A_offset=0, Const=0,
                                EN=1)
            else:
                soma1 = None
            soma2 = p06(addr_in=0x8380, addr_out=0x1f980 >> 2, addr_ciso=0, length_in=length, length_out=length,
    ...
    if phase_en[offset + 5]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            axon = p41(px=7, py=7, cin=128, cout=32, kx=3, ky=3, sx=1, sy=1, addr_ina=0x8300 >> 2,
                       addr_inb=0x10000 >> 2, addr_bias=0x80 >> 2, addr_out=0x19000 >> 2, ina_type=1, inb_type=1,
                       load_bias=2, pad_top=1, pad_down=1, pad_left=1, pad_right=1,
                       data_x=None,
                       data_w=data['layer4.2.conv2']['weight'][(core_x, core_y)],
                       data_b=data['layer4.2.conv2']['bias'][(core_x, core_y)])
    ...
    if phase_en[offset + 6]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            if core_y in [0]:
                ...
                px, py = 12, 1
            axon = p02(avg_pooling_en=False, ina_type=0, load_bias=0, kx=1, ky=4, sx=1, sy=1, cin=32, px=px, py=py,
                       addr_in=0x19000 >> 2, addr_bias=0x0, addr_out=0x8300 >> 2, pad_top=0, pad_down=0, pad_left=0,
                       pad_right=0, bias_length=0, data_x=None, data_b=None, constant_b=0)
            soma1 = None
            router = None
            soma2 = pX5(mode='max', addr_in=0x8300 >> 2, addr_out=0x19000 >> 2, cin=32, cout=32, px=px, py=py, kx=1,
                        ky=1, sx=1, sy=1, cmp_c=0x0, type_in=0, type_out=1, in_cut_start=cuts['layer4.2.cut2'],
                        row_ck_on=0, in_row_max=1)
            phase_group[(chip, (core_x, core_y))]['prims'].append({'axon': axon, 'soma1': soma1, 'router': router,
                                                                   'soma2': soma2})
    ...
    if phase_en[offset + 7]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            if core_y in [0]:
                ...
                px, py = 12, 1
            axon = None
            soma1 = pX5(mode='max', addr_in=0x19000 >> 2, addr_out=0x9000, cin=32, cout=32, px=px, py=py, kx=1,
                        ky=1, sx=1, sy=1, cmp_c=0x0, type_in=1, type_out=1, in_cut_start=0,
                        row_ck_on=0, in_row_max=1)
            addr_rhead_base = MapConfigGen.get_router_rhead_base(phase_group[(chip, (core_x, core_y))]['prims'],
                                                                 limit=0x380)
    ...
    if phase_en[offset + 8]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            router.addRHead(S=0, T=1, P=0, Q=1, X=1 if core_y in [0, 3] else -1, Y=0,
                            A=0, pack_per_Rhead=783, A_offset=0, Const=0, EN=1)
            soma2 = p06(addr_in=0x8380, addr_out=0x19000 >> 2, addr_ciso=0x0000, length_in=7 * 128,
                        length_out=7 * 128, length_ciso=1, num_in=7, num_ciso=7, num_out=7, type_in=1, type_out=1,
                        in_cut_start=0, in_row_max=0, row_ck_on=0, data_in=None, data_ciso=None)
            phase_group[(chip, (core_x, core_y))]['prims'].append({'axon': axon, 'soma1': soma1, 'router': router,
                                                                   'soma2': soma2})

    # L46 卷积，部分和收发 1/3 - 24/49
    if phase_en[offset + 9]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            axon = p41(px=4, py=6, cin=128, cout=128, kx=1, ky=1, sx=1, sy=1, addr_ina=0x19000 >> 2,
                       addr_inb=0x4300 >> 2, addr_bias=0x100 >> 2, addr_out=0x8300 >> 2, ina_type=1, inb_type=1,
                       load_bias=2, pad_top=0, pad_down=0, pad_left=0, pad_right=0,
                       data_x=None,
                       data_w=data['layer4.2.conv3']['weight'][(core_x, core_y)],
                       data_b=data['layer4.2.conv3']['bias'][(core_x, core_y)])
            soma1 = pX5(mode='max', addr_in=0x8300 >> 2, addr_out=0x9000, cin=128, cout=128, px=4, py=6, kx=1,
                        ky=1, sx=1, sy=1, cmp_c=0x80000000, type_in=0, type_out=0, in_cut_start=0, row_ck_on=1,
                        in_row_max=3)
            addr_rhead_base = MapConfigGen.get_router_rhead_base(phase_group[(chip, (core_x, core_y))]['prims'],
                                                                 limit=0x380)
            ...
            soma2 = p06(addr_in=0x8380, addr_out=0x8300 >> 2, addr_ciso=0, length_in=6 * 128, length_out=6 * 128,
                        length_ciso=1, num_in=4, num_ciso=4, num_out=4, type_in=0, type_out=0,
                        in_cut_start=0, in_row_max=0, row_ck_on=0, data_in=None, data_ciso=None)
            phase_group[(chip, (core_x, core_y))]['prims'].append({'axon': axon, 'soma1': soma1, 'router': router,
                                                                   'soma2': soma2})

    # L46 部分和求和，流水Cut， 1/3 - 24/49
    if phase_en[offset + 10]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            axon = p02(avg_pooling_en=False, ina_type=0, load_bias=0, kx=1, ky=4, sx=1, sy=1, cin=128, px=6, py=1,
                       addr_in=0x8300 >> 2, addr_bias=0x0, addr_out=0x1a880 >> 2, pad_top=0, pad_down=0, pad_left=0,
                       pad_right=0, bias_length=0, data_x=None, data_b=None, constant_b=0)
            soma1 = pX5(mode='max', addr_in=0x1a880 >> 2, addr_out=0x8300 >> 2, cin=128, cout=128, px=6, py=1, kx=1,
                        ky=1, sx=1, sy=1, cmp_c=0x80808080, type_in=0, type_out=1, in_cut_start=cuts['layer4.2.cut3'],
    ...
    # L46 卷积，部分和收发 2/3 - 24/49
    if phase_en[offset + 11]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            axon = p41(px=4, py=6, cin=128, cout=128, kx=1, ky=1, sx=1, sy=1, addr_ina=0x19c00 >> 2,
                       addr_inb=0x4300 >> 2, addr_bias=0x100 >> 2, addr_out=0x8600 >> 2, ina_type=1, inb_type=1,
                       load_bias=2, pad_top=0, pad_down=0, pad_left=0, pad_right=0,
                       data_x=None,
                       data_w=None,
                       data_b=None)
            soma1 = pX5(mode='max', addr_in=0x8600 >> 2, addr_out=0x9000, cin=128, cout=128, px=4, py=6, kx=1,
                        ky=1, sx=1, sy=1, cmp_c=0x80000000, type_in=0, type_out=0, in_cut_start=0, row_ck_on=1,
                        in_row_max=3)
            addr_rhead_base = MapConfigGen.get_router_rhead_base(phase_group[(chip, (core_x, core_y))]['prims'],
                                                                 limit=0x380)
            ...
            if core_y == 0:
                ...
                router.addRHead(S=0, T=1, P=0, Q=0, X=0, Y=-1, A=1152, pack_per_Rhead=383, A_offset=0, Const=0, EN=1)
                router.addRHead(S=0, T=1, P=0, Q=0, X=0, Y=0, A=1152, pack_per_Rhead=383, A_offset=0, Const=0, EN=1)
            soma2 = p06(addr_in=0x8380, addr_out=0x8600 >> 2, addr_ciso=0, length_in=6 * 128, length_out=6 * 128,
                        length_ciso=1, num_in=4, num_ciso=4, num_out=4, type_in=0, type_out=0,
                        in_cut_start=0, in_row_max=0, row_ck_on=0, data_in=None, data_ciso=None)
            phase_group[(chip, (core_x, core_y))]['prims'].append({'axon': axon, 'soma1': soma1, 'router': router,
                                                                   'soma2': soma2})

    # L46 部分和求和，流水Cut， 2/3 - 24/49
    if phase_en[offset + 12]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            axon = p02(avg_pooling_en=False, ina_type=0, load_bias=0, kx=1, ky=4, sx=1, sy=1, cin=128, px=6, py=1,
                       addr_in=0x8600 >> 2, addr_bias=0x0, addr_out=0x1a880 >> 2, pad_top=0, pad_down=0, pad_left=0,
                       pad_right=0, bias_length=0, data_x=None, data_b=None, constant_b=0)
            soma1 = pX5(mode='max', addr_in=0x1a880 >> 2, addr_out=0x8600 >> 2, cin=128, cout=128, px=6, py=1, kx=1,
                        ky=1, sx=1, sy=1, cmp_c=0x80808080, type_in=0, type_out=1, in_cut_start=cuts['layer4.2.cut3'],
                        row_ck_on=1, in_row_max=1)
            router = None
            soma2 = None
    ...
    # L46 卷积，部分和收发 3/3 - 1/49
    if phase_en[offset + 13]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            axon = p41(px=1, py=1, cin=128, cout=128, kx=1, ky=1, sx=1, sy=1, addr_ina=0x1a800 >> 2,
                       addr_inb=0x4300 >> 2, addr_bias=0x100 >> 2, addr_out=0x8900 >> 2, ina_type=1, inb_type=1,
                       load_bias=2, pad_top=0, pad_down=0, pad_left=0, pad_right=0,
                       data_x=None,
                       data_w=None,
                       data_b=None)
            soma1 = pX5(mode='max', addr_in=0x8900 >> 2, addr_out=0x9000, cin=128, cout=128, px=1, py=1, kx=1,
                        ky=1, sx=1, sy=1, cmp_c=0x80000000, type_in=0, type_out=0, in_cut_start=0, row_ck_on=1,
                        in_row_max=1)
            addr_rhead_base = MapConfigGen.get_router_rhead_base(phase_group[(chip, (core_x, core_y))]['prims'],
                                                                 limit=0x380)
            ...
            soma2 = p06(addr_in=0x8380, addr_out=0x8900 >> 2, addr_ciso=0, length_in=1 * 128, length_out=1 * 128,
                        length_ciso=1, num_in=4, num_ciso=4, num_out=4, type_in=0, type_out=0,
                        in_cut_start=0, in_row_max=0, row_ck_on=0, data_in=None, data_ciso=None)
            phase_group[(chip, (core_x, core_y))]['prims'].append({'axon': axon, 'soma1': soma1, 'router': router,
                                                                   'soma2': soma2})

    # L46 部分和求和，流水Cut， 3/3 - 1/49
    if phase_en[offset + 14]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            axon = p02(avg_pooling_en=False, ina_type=0, load_bias=0, kx=1, ky=4, sx=1, sy=1, cin=128, px=1, py=1,
                       addr_in=0x8900 >> 2, addr_bias=0x0, addr_out=0x1a880 >> 2, pad_top=0, pad_down=0, pad_left=0,
                       pad_right=0, bias_length=0, data_x=None, data_b=None, constant_b=0)
            soma1 = pX5(mode='max', addr_in=0x1a880 >> 2, addr_out=0x8900 >> 2, cin=128, cout=128, px=1, py=1, kx=1,
                        ky=1, sx=1, sy=1, cmp_c=0x80808080, type_in=0, type_out=1, in_cut_start=cuts['layer4.2.cut3'],
                        row_ck_on=1, in_row_max=1)
            router = None
            soma2 = None
    ...
    # Y46 进行整理，每个得到12*128， 12*128， 12*128， 13*128
    if phase_en[offset + 15]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            if core_y in [0]:
                ...
                px = 12
            soma1 = pX5(mode='max', addr_in=0x8300 >> 2, addr_out=0x9000, cin=128, cout=128, px=px, py=1, kx=1,
                        ky=1, sx=1, sy=1, cmp_c=0x80808080, type_in=1, type_out=1, in_cut_start=0, row_ck_on=0,
    ...
    # L46 + shortcut 部分和求和，流水ReLU，发送给第一行的core，整理成7*7*128
    if phase_en[offset + 16]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            axon = p02(avg_pooling_en=False, ina_type=1, load_bias=0, kx=1, ky=2, sx=1, sy=1, cin=128, px=px, py=1,
                       addr_in=addr_in, addr_bias=0x0, addr_out=0x19000 >> 2, pad_top=0, pad_down=0, pad_left=0,
                       pad_right=0, bias_length=0, data_x=None, data_b=None, constant_b=0)
            soma1 = pX5(mode='max', addr_in=0x19000 >> 2, addr_out=0x9000, cin=128, cout=128, px=px, py=1, kx=1,
                        ky=1, sx=1, sy=1, cmp_c=0x0, type_in=0, type_out=1, in_cut_start=cuts['layer4.2.cut5'],
                        row_ck_on=1, in_row_max=1)
            addr_rhead_base = MapConfigGen.get_router_rhead_base(phase_group[(chip, (core_x, core_y))]['prims'],
                                                                 limit=0x380)
            router = p09(rhead_mode=1, send_en=1, receive_en=0, send_num=px * 128 // 8 - 1, receive_num=3,
                         addr_din_base=0x380, addr_din_length=783, addr_rhead_base=addr_rhead_base,
                         addr_rhead_length=0, addr_dout_base=0x1000, addr_dout_length=0, soma_in_en=1,
                         cxy=0, relay_num=0, data_in=None)
            if core_y == 0:
                router.Receive_en = 1
                router.addRHead(S=0, T=1, P=0, Q=0, X=0, Y=0, A=0, pack_per_Rhead=191, A_offset=0, Const=0, EN=1)
            elif core_y == 1:
                router.addRHead(S=0, T=1, P=0, Q=0, X=0, Y=-1, A=192, pack_per_Rhead=191, A_offset=0, Const=0, EN=1)
            elif core_y == 2:
                router.addRHead(S=0, T=1, P=0, Q=0, X=0, Y=-2, A=384, pack_per_Rhead=191, A_offset=0, Const=0, EN=1)
            else:
                router.addRHead(S=0, T=1, P=0, Q=0, X=0, Y=-3, A=576, pack_per_Rhead=207, A_offset=0, Const=0, EN=1)
            if core_y == 0:
                soma2 = p06(addr_in=0x8380, addr_out=0xe780 >> 2, addr_ciso=0, length_in=7 * 128, length_out=7 * 128,
                            length_ciso=1, num_in=7, num_ciso=7, num_out=7, type_in=1, type_out=1,
                            in_cut_start=0, in_row_max=0, row_ck_on=0, data_in=None, data_ciso=None)
            else:
                soma2 = None
    ...
    # Average Pooling，流水Cut
    if phase_en[offset + 17]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            axon = p02(avg_pooling_en=True, ina_type=1, load_bias=0, kx=7, ky=7, sx=1, sy=1, cin=128, px=7, py=7,
                       addr_in=0xe780 >> 2, addr_bias=0x0, addr_out=0x19000 >> 2, pad_top=0, pad_down=0, pad_left=0,
                       pad_right=0, bias_length=0, data_x=None, data_b=None, constant_b=0)
            soma1 = pX5(mode='max', addr_in=0x19000 >> 2, addr_out=0x1f200 >> 2, cin=128, cout=128, px=1, py=1, kx=1,
                        ky=1, sx=1, sy=1, cmp_c=0x80808080, type_in=0, type_out=1, in_cut_start=cuts['avgpool_cut'],
                        row_ck_on=1, in_row_max=1)
            if core_y != 0:
...
if __name__ == '__main__':
    ...
    phase[offset + 0] = 1  # L44 卷积，部分和收发
    phase[offset + 1] = 1  # L44 部分和求和，流水ReLU，发送整理
    phase[offset + 2] = 1  # L44 输出整理，横向多播部分

    phase[offset + 3] = 1  # X 整理，得到Xe

    phase[offset + 4] = 1  # L45 卷积，部分和收发
    phase[offset + 5] = 1  # L45 部分和求和，流水ReLU，发送整理
    phase[offset + 6] = 1  # L45 输出整理，横向多播部分

    phase[offset + 7] = 1  # L41e 卷积，部分和收发 1/3 - 24/49
    phase[offset + 8] = 1  # L41e 部分和求和，流水Cut， 1/3 - 24/49
    phase[offset + 9] = 1  # L41e 卷积，部分和收发 2/3 - 24/49
    phase[offset + 10] = 1  # L41e 部分和求和，流水Cut， 2/3 - 24/49
    phase[offset + 11] = 1  # L41e 卷积，部分和收发 3/3 - 1/49
    phase[offset + 12] = 1  # L41e 部分和求和，流水Cut， 3/3 - 1/49

    phase[offset + 13] = 1  # Ye 进行整理，每个得到12*128， 12*128， 12*128， 13*128

    phase[offset + 14] = 1  # L46 + shortcut 部分和求和，流水ReLU，发送给第一行的core，整理成7*7*128

    phase[offset + 15] = 1  # Average Pooling，流水Cut
    ...
    data = generate_g17_data(handler, size_y=4, size_x=16)

    clock_in_phase = 100_000
    config = gen_g17_map_config(phase, clock_in_phase=clock_in_phase, size_x=16, size_y=4, data=data, in_data_en=False,
                                out_data_en=False, chip=chip, cuts=cuts, delay_l4=delay, delay_l5=delay)
