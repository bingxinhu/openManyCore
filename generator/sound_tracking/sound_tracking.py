...
from generator.sound_tracking.GRU import GRU, cut, GRUCell
...
if __name__ == '__main__':
    ...
    from generator.sound_tracking.sound_tracking_fp32 import ReferenceModel
    from generator.sound_tracking.sound_tracking_fp32 import snn_n_model
    from generator.sound_tracking.utils import fuse2d_conv_bn

    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    reference = ReferenceModel(input_channels, output_channels, hidden_size)
    # snn = snn_n_model()
    # reference.snn = snn.to(device)
    state_dict = torch.load('generator/sound_tracking/sound_tracking.pth', 
                            map_location=device)
