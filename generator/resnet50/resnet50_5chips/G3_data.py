...
def generate_g3_data(handler, size_y, size_x):
    ...
    l5_input_raw_data = handler.parameters['layer1.1.conv1']['input'].astype(np.int8)
    l5_weight_raw_data = handler.parameters['layer1.1.conv1']['weight'].astype(np.int8)
    l5_bias_raw_data = handler.parameters['layer1.1.conv1']['bias'].astype(np.int32)
    l5_output_raw_data = handler.parameters['layer1.1.cut1']['output'].astype(np.int8)
    ...
    data['layer1.1.conv1']['bias'] = ResNetDataHandler.tensor_split(
        raw_data=l5_bias_raw_data, split_dict=l5_bias_split_dict, data_type=0, alignment=(32,), dims=(0,))
    data['layer1.1.cut1']['output'] = ResNetDataHandler.tensor_split(
    ...
    l6_weight_raw_data = handler.parameters['layer1.1.conv2']['weight'].astype(np.int8)
    l6_bias_raw_data = handler.parameters['layer1.1.conv2']['bias'].astype(np.int32)
    l6_output_raw_data = handler.parameters['layer1.1.cut2']['output'].astype(np.int8)
    ...
    l7_input_split_dict, l7_weight_split_dict, l7_bias_split_dict, l7_out_32_split, l7_output_split_dict = \
        {}, {}, {}, {}, {}
    l7_input_raw_data = handler.parameters['layer1.1.conv3']['input'].astype(np.int8)
    l7_weight_raw_data = handler.parameters['layer1.1.conv3']['weight'].astype(np.int8)
    l7_bias_raw_data = handler.parameters['layer1.1.conv3']['bias'].astype(np.int32)
    l7_output_raw_data = handler.parameters['layer1.1.cut3']['output'].astype(np.int8)
    l7_out_32_raw_data = handler.parameters['layer1.1.conv3']['output'].astype(np.int32)
    ...
    y_c3_raw_data = handler.parameters['layer1.1.cut3']['output'].astype(np.int8)
    y_output_raw_data = handler.parameters['layer1.1.cut5']['output'].astype(np.int8)
    ...
    for core_y, core_x in product(range(size_y), range(size_x)):
        y_c3_split_dict[(core_x, core_y)] = ((0, y_oc), (core_y * 2 + core_x * 4, core_y * 2 + core_x * 4 + 2),
                                             (0, y_ow))
        y_output_split_dict[(core_x, core_y)] = ((0, y_oc), (core_y * 2 + core_x * 4, core_y * 2 + core_x * 4 + 2),
                                                 (0, y_ow))

        y_output_split_dict1[(core_x, core_y)] = ((0, 256),
                                                  ((core_x * 2 + core_y) * 112, (core_x * 2 + core_y) * 112 + 28),
                                                  (0, 1))
        y_output_split_dict2[(core_x, core_y)] = ((0, 256),
                                                  ((core_x * 2 + core_y) * 112 + 28, (core_x * 2 + core_y) * 112 + 56),
                                                  (0, 1))
        y_output_split_dict3[(core_x, core_y)] = ((0, 256),
                                                  ((core_x * 2 + core_y) * 112 + 56, (core_x * 2 + core_y) * 112 + 84),
                                                  (0, 1))
        y_output_split_dict4[(core_x, core_y)] = ((0, 256),
                                                  ((core_x * 2 + core_y) * 112 + 84, (core_x * 2 + core_y) * 112 + 112),
                                                  (0, 1))
    data['layer1.1.cut3']['output2'] = ResNetDataHandler.tensor_split(
        raw_data=y_c3_raw_data, split_dict=y_c3_split_dict, data_type=1, alignment=(16, None, None),
        dims=(0, 2, 1))
    data['layer1.1.cut5']['output'] = ResNetDataHandler.tensor_split(
        raw_data=y_output_raw_data, split_dict=y_output_split_dict, data_type=1, alignment=(16, None, None),
        dims=(0, 2, 1))

    data['layer1.1.cut5']['output1'] = ResNetDataHandler.tensor_split(
        raw_data=y_output_raw_data.reshape((256, -1, 1)), split_dict=y_output_split_dict1, data_type=1,
        alignment=(16, None, None), dims=(0, 2, 1))
    data['layer1.1.cut5']['output2'] = ResNetDataHandler.tensor_split(
        raw_data=y_output_raw_data.reshape((256, -1, 1)), split_dict=y_output_split_dict2, data_type=1,
        alignment=(16, None, None), dims=(0, 2, 1))
    data['layer1.1.cut5']['output3'] = ResNetDataHandler.tensor_split(
        raw_data=y_output_raw_data.reshape((256, -1, 1)), split_dict=y_output_split_dict3, data_type=1,
        alignment=(16, None, None), dims=(0, 2, 1))
    data['layer1.1.cut5']['output4'] = ResNetDataHandler.tensor_split(
        raw_data=y_output_raw_data.reshape((256, -1, 1)), split_dict=y_output_split_dict4, data_type=1,
        alignment=(16, None, None), dims=(0, 2, 1))
