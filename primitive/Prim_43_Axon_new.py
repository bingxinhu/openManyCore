...
class Prim_43_Axon(Primitive):
    ...
    def init_data(self) -> list:
        ...
        if self.Load_Bias == 0 or self.Load_Bias == 1:
            ...
            self.Bias_array = data_generator.random_array_32(self.Bias_size).astype(np.int64)

        self.Input_A_size = self.InB_length
        if self.InB_type == 2:
            self.A_array = data_generator.random_array_u8(self.Input_A_size).astype(np.int64)
        else:
            self.A_array = data_generator.random_array_8(self.Input_A_size).astype(np.int64)

        if self.InA_type == 1 or self.InA_type == 2:
            self.cin_wr_real = (math.ceil(self.InB_length / 32)) * 32
            num_in_4B = 4
        else:
            self.cin_wr_real = (math.ceil(self.InB_length / 128)) * 128
            num_in_4B = 16
        ...
        _data = []
        for i in range(int(self.InB_length / 4)):
            _tmp = []
            for j in range(4):
                _tmp.append(int(self.A_array[4 * i + j]))
                pass
            _data.append(_tmp)
            pass
        _retLsit.append(_data)
    ...
    def getInfoList(self) -> list:
        ...
        self.Read_A_length = (math.ceil(self.InB_length / 32)) * 8
        ...
        if self.InA_type == 1 or self.InA_type == 2:
            ...
            if self.tensor_en:
                self.Read_X_length = self.Py * self.Px * (math.ceil(self.cin_wr_real / 128) * 8)
            else:
                self.Read_X_length = self.X_array_num * (math.ceil(self.cin_wr_real / 128) * 8)
    ...
    def convertMem2Prim(self, inputList: list):
        if self.InA_type == 1 or self.InA_type == 2:
            self.Km_num = np.ceil(self.InB_length / 32).astype(int)
            self.cin_wr_real = (math.ceil(self.InB_length / 32)) * 32
            num_in_4B = 4
        else:
            self.Km_num = np.ceil(self.InB_length / 128).astype(int)
            self.cin_wr_real = (math.ceil(self.InB_length / 128)) * 128
            num_in_4B = 16
        ...
        self.Input_A_size = self.InB_length
        self.A_array = np.zeros(self.Input_A_size).astype(np.int64)
        for i in range(int(self.InB_length / 4)):
            for j in range(4):
                self.A_array[4 * i + j] = inputList[1][i][j]
            pass
        ...
        if self.tensor_en:
            ...
            if self.Load_Bias == 0 or self.Load_Bias == 1:
                for oy_cnt in range(self.Oy):
                    for ox_cnt in range(self.Ox):
                        for cin_cnt in range(self.cin):
                            self.result[oy_cnt][ox_cnt][cin_cnt] = self.constant_b + self.A_array[cin_cnt] * \
                                                                   self.InputX_array[oy_cnt * self.stride_y][ox_cnt * self.stride_x][cin_cnt]
                ...
                for oy_cnt in range(self.Oy):
                    for ox_cnt in range(self.Ox):
                        for cin_cnt in range(self.cin):
                            self.result[oy_cnt][ox_cnt][cin_cnt] = self.Bias_array[cin_cnt] + self.A_array[cin_cnt] * \
                                                                   self.InputX_array[oy_cnt * self.stride_y][ox_cnt * self.stride_x][cin_cnt]
                            if self.result[oy_cnt][ox_cnt][cin_cnt] >= 0x7fffffff:
                                self.result[oy_cnt][ox_cnt][cin_cnt] = 0x7fffffff
                            elif self.result[oy_cnt][ox_cnt][cin_cnt] <= -0x80000000:
            ...
            if self.Load_Bias == 0 or self.Load_Bias == 1:
                for i in range(self.X_array_num):
                    for cin_cnt in range(self.cin):
                        self.result[i][cin_cnt] = self.constant_b + self.A_array[cin_cnt] * \
                ...
                for i in range(self.X_array_num):
                    for cin_cnt in range(self.cin):
                        self.result[i][cin_cnt] = self.Bias_array[cin_cnt] + self.A_array[cin_cnt] * \
                                                  self.InputX_array[i][cin_cnt]
                        if self.result[i][cin_cnt] >= 0x7fffffff:
                            self.result[i][cin_cnt] = 0x7fffffff
                        elif self.result[i][cin_cnt] <= -0x80000000:
    ...
    def cal_para(self):
        self.Oy = int((self.Py - 1) / self.stride_y) + 1
        self.Ox = int((self.Px - 1) / self.stride_x) + 1
        self.L3_num = math.ceil(self.InB_length / 32) - 1
        self.Read_A_length = (math.ceil(self.InB_length / 32)) * 8
        ...
        if self.InA_type == 1 or self.InA_type == 2:
            ...
            if self.tensor_en:
                self.Read_X_length = self.Py * self.Px * (math.ceil(self.cin_wr_real / 128) * 8)
                self.Write_V_length = int(self.Oy * self.Ox * self.cin)
            else:
                self.Read_X_length = self.X_array_num * (math.ceil(self.cin_wr_real / 128) * 8)
                self.Write_V_length = int(self.X_array_num * self.cin)
    ...
    def save_para(self, path: str):
        with open(path, 'w') as f:
            ...
            f.write('Read_A_length = ' + str(self.Read_A_length) + '\n')
