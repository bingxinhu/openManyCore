...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = conv1x1(inplanes, width, stride=stride, bias=not batch_norm)
        self.relu1 = nn.ReLU(inplace=True)
        self.cut1 = Cut(en=self.quantization)

        if self.batch_norm:
            self.bn1 = norm_layer(width)
            self.bn2 = norm_layer(width)
            self.bn3 = norm_layer(planes * self.expansion)
        self.conv2 = conv3x3(width, width, groups, dilation, bias=not batch_norm)
        self.relu2 = nn.ReLU(inplace=True)
        self.cut2 = Cut(en=self.quantization)

        self.conv3 = conv1x1(width, planes * self.expansion, bias=not batch_norm)
        self.cut3 = Cut(en=self.quantization)

        self.downsample = downsample
        if downsample is not None:
            self.cut4 = Cut(en=self.quantization)

        self.relu3 = nn.ReLU(inplace=True)
        self.cut5 = Cut(en=self.quantization, in_cut_start=1)
...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        assert (batch_norm ^ quantization)
        ...
        self.relu = nn.ReLU(inplace=True)
        self.conv1_cut = Cut(en=self.quantization)
        ...
        self.avgpool_cut = Cut(en=self.quantization)
        self.fc = nn.Linear(512 * block.expansion, num_classes)
        self.fc_cut = Cut(en=self.quantization)
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        _, _, ph, pw = x.shape
        # 会导致Avg的输入大，不能在check里使用，且量化时需要注意fc的权重变小 TODO
        x = x * ph * pw
...
def _resnet(
    ...
    model = ResNet(block, layers, batch_norm=batch_norm, quantization=quantization, **kwargs)
    if pretrained:
    ...
    if quantization:
        ...
        #     if 'bias' in name:
        #         p.data.mul_(128 * 128)
        #         p.data.floor_().clamp_(min=-128 * 128, max=128 * 128)
        #     else:
        #         assert ('weight' in name)
        #         p.data.mul_(128)
        #         p.data.floor_().clamp_(min=-128, max=127)
        # cut parameters
        # from generator.resnet50.quantization_config import QuantizationConfig     # TODO
        from generator.resnet50.resnet50_imagenet_quantization_config import QuantizationConfig
        in_cut_start_dict = QuantizationConfig('in_cut_start')
...
def resnet50(pretrained: bool = False, progress: bool = True, batch_norm: bool = True, quantization: bool = False,
             **kwargs: Any) -> ResNet:
    ...
    return _resnet('resnet50', Bottleneck, [3, 4, 6, 3], pretrained, progress,
                   batch_norm=batch_norm, quantization=quantization, **kwargs)
...
if __name__ == '__main__':
    import numpy as np
    torch.manual_seed(1)
    model = resnet50(pretrained=False, progress=True, batch_norm=False, quantization=True)
    print(model)
