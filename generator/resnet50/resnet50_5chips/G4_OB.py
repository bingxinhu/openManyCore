...
def gen_g4_ob_map_config(phase_en, clock_in_phase, data=None,
                         in_data_en=False, out_data_en=False, delay_l4=None, delay_l5=None, init_data=None):
    """
        ResNet-50 5-Chip Group-4 Output Buffer
        core array : chip(0, 0)   [(9, 9), (10, 9), (11, 9), (12, 9), (13, 9), (14, 9), (15, 9)]
                     chip(1, 0)   [(1, 9), (2, 9), (3, 9)]
                     chip(1, 1)   [(1, 0), (2, 0), (3, 0)]
    """
    map_config = {
        'sim_clock': None,
        ((0, 0), 0): {
    ...
    for core_x, core_y in [(9, 9), (10, 9), (11, 9), (12, 9), (13, 9), (14, 9), (15, 9)]:
        map_config[((0, 0), 0)][0][((0, 0), (core_x, core_y))] = {
            'prims': []
        }
    for core_x, core_y in [(1, 9), (2, 9), (3, 9)]:
        map_config[((1, 0), 0)][1][((1, 0), (core_x, core_y))] = {
            'prims': []
        }
    for core_x, core_y in [(1, 0), (2, 0), (3, 0)]:
        map_config[((1, 1), 0)][2][((1, 1), (core_x, core_y))] = {
            'prims': []
        }
    phase_group_0 = map_config[((0, 0), 0)][0]
    phase_group_1 = map_config[((1, 0), 0)][1]
    phase_group_2 = map_config[((1, 1), 0)][2]
    ...
    if phase_en[0]:
        for core_x, core_y in [(9, 9), (10, 9), (11, 9), (12, 9), (13, 9), (14, 9), (15, 9)]:
            ...
            if out_data_en:
                soma1 = pX5(mode='max', addr_in=0x7000 >> 2, addr_out=0x9000, cin=256, cout=256, px=4, py=7, kx=1,
                            ...
                            in_row_max=1, data_in=data['conv3a1_input1'][(core_x - 9, 0)] if init_data else None)
            addr_rhead_base = MapConfigGen.get_router_rhead_base(phase_group_0[((0, 0), (core_x, core_y))]['prims'],
                                                                 limit=0x380)
            router = p09(rhead_mode=1, send_en=out_data_en,
                         receive_en=in_data_en, send_num=28 * 256 // 8 - 1, receive_num=0,
            ...
            if out_data_en:
                if core_x in [9, 10, 11, 12]:
                    if core_x in [9, 10]:
                        dst_x, dst_y = 14, 12 + core_x - 9
                    else:
                        dst_x, dst_y = 15, 12 + core_x - 11
                elif core_x in [13, 14, 15]:
                    dst_x, dst_y = 17 + core_x - 13, 9
                else:
                    raise ValueError
                router.addRHead(S=0, T=1, P=0, Q=1, X=dst_x - core_x, Y=dst_y - core_y, A=0,
                                pack_per_Rhead=28 * 256 // 8 - 1, A_offset=0, Const=0, EN=1)
            ...
            if not in_data_en:
                soma2 = None
            phase_group_0[((0, 0), (core_x, core_y))]['prims'].append({'axon': axon, 'soma1': soma1, 'router': router,
                                                                       'soma2': soma2})
        ...
        for core_x, core_y in [(1, 9), (2, 9), (3, 9)]:
            ...
            router = p09(rhead_mode=1, send_en=0,
                         ...
                         addr_rhead_length=0, addr_dout_base=0x1000, addr_dout_length=0, soma_in_en=1,
                         cxy=out_data_en, relay_num=28 * 256 // 8 - 1, nx=0, ny=1, data_in=None)
            soma2 = None
            phase_group_1[((1, 0), (core_x, core_y))]['prims'].append({'axon': axon, 'soma1': soma1, 'router': router,
                                                                       'soma2': soma2})
        for core_x, core_y in [(1, 0), (2, 0), (3, 0)]:
            axon = p41(px=1, py=1, load_bias=0, cin=1, cout=1, kx=1, ky=1, sx=1, sy=1, addr_ina=0x10000 >> 2,
                       addr_inb=0x10000 >> 2, addr_bias=0x10000 >> 2, addr_out=0x10000 >> 2, axon_delay=True,
            ...
            router = p09(rhead_mode=1, send_en=0,
                         ...
                         addr_din_base=0x380, addr_din_length=28*256//8-1, addr_rhead_base=addr_rhead_base,
                         addr_rhead_length=0, addr_dout_base=0x1000, addr_dout_length=0, soma_in_en=1,
                         cxy=out_data_en, relay_num=28*256//8-1, nx=dst_x - core_x, ny=dst_y - core_y, data_in=None)
            soma2 = None
            phase_group_2[((1, 1), (core_x, core_y))]['prims'].append({'axon': axon, 'soma1': soma1, 'router': router,
                                                                       'soma2': soma2})
        ...
        for core_x, core_y in [(9, 9), (10, 9), (11, 9), (12, 9), (13, 9), (14, 9), (15, 9)]:
            ...
            if out_data_en:
                soma1 = pX5(mode='max', addr_in=0x8c00 >> 2, addr_out=0x9000, cin=256, cout=256, px=4, py=7, kx=1,
                            ky=1, sx=1, sy=1, cmp_c=0x80808080, type_in=1, type_out=1, in_cut_start=0, row_ck_on=0,
                            in_row_max=1, data_in=data['conv3a1_input2'][(core_x - 9, 0)] if init_data else None)
            addr_rhead_base = MapConfigGen.get_router_rhead_base(phase_group_0[((0, 0), (core_x, core_y))]['prims'],
                                                                 limit=0x380)
            router = p09(rhead_mode=1, send_en=out_data_en,
                         receive_en=in_data_en, send_num=28*256//8-1, receive_num=0,
        ...
        for core_x, core_y in [(9, 9), (10, 9), (11, 9), (12, 9), (13, 9), (14, 9), (15, 9)]:
            ...
            if out_data_en:
                soma1 = pX5(mode='max', addr_in=0xa800 >> 2, addr_out=0x9000, cin=256, cout=256, px=4, py=7, kx=1,
                            ky=1, sx=1, sy=1, cmp_c=0x80808080, type_in=1, type_out=1, in_cut_start=0, row_ck_on=0,
                            in_row_max=1, data_in=data['conv3a1_input3'][(core_x - 9, 0)] if init_data else None)
            addr_rhead_base = MapConfigGen.get_router_rhead_base(phase_group_0[((0, 0), (core_x, core_y))]['prims'],
                                                                 limit=0x380)
            router = p09(rhead_mode=1, send_en=out_data_en,
                         receive_en=in_data_en, send_num=28*256//8-1, receive_num=0,
        ...
        for core_x, core_y in [(9, 9), (10, 9), (11, 9), (12, 9), (13, 9), (14, 9), (15, 9)]:
            ...
            if out_data_en:
                soma1 = pX5(mode='max', addr_in=0xc400 >> 2, addr_out=0x9000, cin=256, cout=256, px=4, py=7, kx=1,
                            ky=1, sx=1, sy=1, cmp_c=0x80808080, type_in=1, type_out=1, in_cut_start=0, row_ck_on=0,
                            in_row_max=1, data_in=data['conv3a1_input4'][(core_x - 9, 0)] if init_data else None)
            addr_rhead_base = MapConfigGen.get_router_rhead_base(phase_group_0[((0, 0), (core_x, core_y))]['prims'],
                                                                 limit=0x380)
            router = p09(rhead_mode=1, send_en=out_data_en,
                         receive_en=in_data_en, send_num=28*256//8-1, receive_num=0,
