...
def main():
    case_file_name = 'Det_074'

    config = MapConfigGen()
    obstacle_handler = DetectionDataHandler(name='obstacle', pretrained=False)
    obstacle_data_all = detection_data(handler=obstacle_handler)
    mouse_handler = DetectionDataHandler(name='mouse', pretrained=False)
    ...
    # chip_group = config.map_config[((0, 0), 0)]
    # config.map_config[((0, 0), 1)] = {}
    # config.map_config[((0, 0), 1)][7] = chip_group.pop(7)
    # config.map_config[((0, 0), 1)][8] = chip_group.pop(8)
    # config.map_config[((0, 0), 1)][9] = chip_group.pop(9)
    # config.map_config[((0, 0), 1)][10] = chip_group.pop(10)
    # config.map_config[((0, 0), 1)][11] = chip_group.pop(11)
    # config.map_config[((0, 0), 1)][12] = chip_group.pop(12)
