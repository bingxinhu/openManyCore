...
def get_core_id(core_x, core_y):
    core_id = core_x + core_y * 4
    return core_id


def gen_LP_map_config(phase_en, clock_in_phase, size_x, size_y, delay_l4=None, delay_l5=None,
    ...
    if phase_en[offset + 0]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            axon = p04(cin=16, cout=128, addr_ina=0x0000 >> 2, addr_inb=0x10000 >> 2, addr_bias=0x10800 >> 2,
    ...
    if phase_en[offset + 1]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            axon = p04(cin=128, cout=32, addr_ina=0x0020 >> 2, addr_inb=0x10A00 >> 2, addr_bias=0x11A00 >> 2,
                       addr_out=0x00A0 >> 2, ina_type=1, inb_type=1, load_bias=2,
            ...
            axon = p41(px=1, py=1, cin=axon.cin, cout=axon.cout, kx=1, ky=1, sx=1, sy=1,
                       ...
                       data_x=None, data_w=[], data_b=[])
            soma1 = p06(addr_in=0x00A0 >> 2, addr_out=0x24000 >> 2,
            ...
            router = p09(rhead_mode=1, send_en=False if (core_x == 1 and core_y == 1) else True,
                         receive_en=True if (core_x == 1 and core_y == 1) else False, send_num=4 * 32 // 8 - 1,
                         receive_num=15 - 1,
            ...
            if not (core_x == 1 and core_y == 1):
                router.addRHead(S=0, T=1, P=0, Q=0, X=1 - core_x, Y=1 - core_y,
                                A=4 * 32 // 8 * (get_core_id(core_x, core_y) - 0) if get_core_id(core_x, core_y) < 5
                                else 4 * 32 // 8 * (get_core_id(core_x, core_y) - 1),
                                pack_per_Rhead=4 * 32 // 8 - 1,
                                A_offset=0, Const=0, EN=1)
            if core_x == 1 and core_y == 1:
                soma1 = None
                soma2 = p06(addr_in=0x21000 >> 2, addr_out=0x0120 >> 2,
                            addr_ciso=0x0000 >> 2, length_in=32,
                            length_out=32, length_ciso=1, num_in=15, num_ciso=15, num_out=15, type_in=0,
    ...
    if phase_en[offset + 2]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            if core_x == 1 and core_y == 1:
                axon = p02(avg_pooling_en=False, ina_type=0, load_bias=0, kx=4, ky=4, sx=1, sy=1, cin=32, px=1, py=1,
                           addr_in=0x00A0 >> 2, addr_bias=0x0, addr_out=0x8000 >> 2, pad_top=0, pad_down=0, pad_left=0,
                           pad_right=0, bias_length=0, data_x=None, data_b=None, constant_b=0)
                soma1 = pX5(mode='max', addr_in=0x8000 >> 2, addr_out=0x08A0 >> 2,
...
if __name__ == '__main__':
    ...
    config = gen_LP_map_config(phase, clock_in_phase=clock_in_phase, size_x=4, size_y=4,
