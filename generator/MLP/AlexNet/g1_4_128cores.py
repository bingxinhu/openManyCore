...
def gen_1_map_config(phase_en, clock_in_phase, size_x, size_y, delay_l4=None, delay_l5=None,
    ...
    if phase_en[0]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            soma1 = p06(addr_in=0x8000 >> 2, addr_out=0x24000 >> 2,
                        addr_ciso=0x0000, length_in=2048,
                        length_out=2048, length_ciso=1, num_in=1, num_ciso=1, num_out=1, type_in=1,
            ...
            router = p09(rhead_mode=1, send_en=out_data_en,
                         receive_en=in_data_en, send_num=2048 // 8 - 1, receive_num=0,
                         addr_din_base=0x1000 >> 2, addr_din_length=2048 // 8 - 1, addr_rhead_base=addr_rhead_base,
                         addr_rhead_length=0, addr_dout_base=0x1000, addr_dout_length=0, soma_in_en=1,
                         cxy=0, relay_num=0, nx=0, ny=0, data_in=None)
            router.addRHead(S=0, T=1, P=0, Q=0, X=0, Y=0, A=0, pack_per_Rhead=2048 // 8 - 1, A_offset=0, Const=0, EN=1)
            soma2 = p06(addr_in=0x21000 >> 2, addr_out=0x0000 >> 2,
                        addr_ciso=0x10000 >> 2, length_in=2048,
                        length_out=2048, length_ciso=1, num_in=1, num_ciso=1, num_out=1, type_in=1,
                        type_out=1, in_cut_start=0, in_row_max=0, row_ck_on=0,
    ...
    if phase_en[offset + 0]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            axon = p04(cin=2048, cout=32, addr_ina=0x0000 >> 2, addr_inb=0x10000 >> 2, addr_bias=0xFF80 >> 2,
                       addr_out=0x0800 >> 2, ina_type=1, inb_type=1, load_bias=2,
                       data_x=None, data_w=[], data_b=[])
            soma1 = p06(addr_in=0x0800 >> 2, addr_out=0x24000 >> 2,
            ...
            router = p09(rhead_mode=1, send_en=False if core_x == 0 else True,
                         receive_en=True if core_x == 0 else False, send_num=4 * 32 // 8 - 1, receive_num=3 - 1,
                         addr_din_base=0x1000 >> 2, addr_din_length=3 * 4 * 32 // 8 - 1, 
                         addr_rhead_base=addr_rhead_base,
                         addr_rhead_length=0, addr_dout_base=0x1000, addr_dout_length=0, soma_in_en=1,
                         cxy=0, relay_num=0, nx=0, ny=0, data_in=None)
            if core_x != 0:
                router.addRHead(S=0, T=1, P=0, Q=0, X=-core_x, Y=0, A=4 * 32 // 8 * (core_x - 1), pack_per_Rhead=4 * 32 // 8 - 1, 
                                A_offset=0, Const=0, EN=1)
            if core_x == 0:
                soma1 = None
                soma2 = p06(addr_in=0x21000 >> 2, addr_out=0x0880 >> 2,
                            addr_ciso=0x0000 >> 2, length_in=3 * 32,
                            length_out=3 * 32, length_ciso=1, num_in=1, num_ciso=1, num_out=1, type_in=0,
    ...
    if phase_en[offset + 1]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            axon = p02(avg_pooling_en=False, ina_type=0, load_bias=0, kx=1, ky=4, sx=1, sy=1, cin=32, px=1, py=1,
                       addr_in=0x0800 >> 2, addr_bias=0x0, addr_out=0x1000 >> 2, pad_top=0, pad_down=0, pad_left=0,
                       pad_right=0, bias_length=0, data_x=None, data_b=None, constant_b=0)
            soma1 = pX5(mode='max', addr_in=0x1000 >> 2, addr_out=0x8000 >> 2,
...
if __name__ == '__main__':
    ...
    config = gen_1_map_config(phase, clock_in_phase=clock_in_phase, size_x=4, size_y=10,
