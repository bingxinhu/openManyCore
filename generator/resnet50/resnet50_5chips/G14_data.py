...
def generate_g14_data(handler, size_y, size_x):
    data = {
        'layer3.5.conv1': {},  # L26
        'layer3.5.relu1': {},
        'layer3.5.cut1': {},
    ...
    l1_input_raw_data = handler.parameters['layer3.5.conv1']['input'].astype(np.int8)
    l1_weight_raw_data = handler.parameters['layer3.5.conv1']['weight'].astype(np.int8)
    l1_bias_raw_data = np.append(handler.parameters['layer3.5.conv1']['bias'].astype(np.int32),
                                 np.zeros_like(handler.parameters['layer3.5.conv1']['bias']).astype(np.int32))
    l1_output_raw_data = handler.parameters['layer3.5.cut1']['output'].astype(np.int8).reshape((256, -1, 1))
    ...
    for core_y, core_x in product(range(size_y), range(size_x)):
        ...
        l1_weight_split_dict[(core_x, core_y)] = ((core_x * 32, core_x * 32 + 32), (core_y * 256, core_y * 256 + 256),
                                                  (0, l1_wkh), (0, l1_wkw))
        l1_input_split_dict5[(core_x, core_y)] = ((core_y * 256, core_y * 256 + 256),
                                                  (0, 7),
                                                  (0, 7))
        l1_input_split_dict6[(core_x, core_y)] = ((core_y * 256, core_y * 256 + 256),
                                                  (7, 14),
                                                  (0, 7))
        l1_input_split_dict7[(core_x, core_y)] = ((core_y * 256, core_y * 256 + 256),
                                                  (14, 21),
                                                  (0, 7))
        l1_input_split_dict8[(core_x, core_y)] = ((core_y * 256, core_y * 256 + 256),
                                                  (21, 28),
                                                  (0, 7))
        if core_y == 0:
        ...
        l1_input_split_dict3[(core_x, core_y)] = ((core_x // 2 * 64 + core_y * 256,
                                                  ...
                                                  (0, l1_iw))  # ADD input
    data['layer3.5.conv1']['input1'] = ResNetDataHandler.tensor_split(
        raw_data=l1_input_raw_data, split_dict=l1_input_split_dict1, data_type=1, alignment=(16, None, None),
        dims=(0, 2, 1))
    data['layer3.5.conv1']['input2'] = ResNetDataHandler.tensor_split(
        raw_data=l1_input_raw_data, split_dict=l1_input_split_dict2, data_type=1, alignment=(16, None, None),
        dims=(0, 2, 1))
    data['layer3.5.conv1']['input3'] = ResNetDataHandler.tensor_split(
        raw_data=l1_input_raw_data, split_dict=l1_input_split_dict3, data_type=1, alignment=(16, None, None),
        dims=(0, 2, 1))
    data['layer3.5.conv1']['input5'] = ResNetDataHandler.tensor_split(
        raw_data=l1_input_raw_data.reshape((1024, 28, 7)), split_dict=l1_input_split_dict5,
        data_type=1, alignment=(16, None, None), dims=(0, 2, 1))
    data['layer3.5.conv1']['input6'] = ResNetDataHandler.tensor_split(
        raw_data=l1_input_raw_data.reshape((1024, 28, 7)), split_dict=l1_input_split_dict6,
        data_type=1, alignment=(16, None, None), dims=(0, 2, 1))
    data['layer3.5.conv1']['input7'] = ResNetDataHandler.tensor_split(
        raw_data=l1_input_raw_data.reshape((1024, 28, 7)), split_dict=l1_input_split_dict7,
        data_type=1, alignment=(16, None, None), dims=(0, 2, 1))
    data['layer3.5.conv1']['input8'] = ResNetDataHandler.tensor_split(
        raw_data=l1_input_raw_data.reshape((1024, 28, 7)), split_dict=l1_input_split_dict8,
        data_type=1, alignment=(16, None, None), dims=(0, 2, 1))
    data['layer3.5.conv1']['weight'] = ResNetDataHandler.tensor_split(
    ...
    l2_weight_raw_data = handler.parameters['layer3.5.conv2']['weight'].astype(np.int8)
    l2_bias_raw_data = np.append(handler.parameters['layer3.5.conv2']['bias'].astype(np.int32),
                                 np.zeros_like(handler.parameters['layer3.5.conv2']['bias'], dtype=np.int32))
    l2_output_raw_data = handler.parameters['layer3.5.cut2']['output'].astype(np.int8).reshape((256, -1, 1))
    ...
    l3_input_raw_data = handler.parameters['layer3.5.conv3']['input'].astype(np.int8).reshape(256, 28, 7)
    l3_weight_raw_data = handler.parameters['layer3.5.conv3']['weight'].astype(np.int8)
    l3_bias_raw_data = np.append(handler.parameters['layer3.5.conv3']['bias'].astype(np.int32),
                                 np.zeros_like(handler.parameters['layer3.5.conv3']['bias'], dtype=np.int32))
    l3_output_raw_data = handler.parameters['layer3.5.cut3']['output'].astype(np.int8)
    ...
    y_output_split_dict, y_output_split_dict1 = {}, {}
    y_output_raw_data = handler.parameters['layer3.5.cut5']['output'].astype(np.int8)  # 1024, 14, 14
    ...
    for core_y, core_x in product(range(size_y), range(size_x)):
        ...
        if core_x % 2 == 0:
            ...
            y_output_split_dict1[(core_x, core_y)] = ((core_x // 2 * 64 + core_y * 256,
                                                      ...
                                                      (4, 7),
                                                      (0, l3_ow))
    data['layer3.5.cut5']['output'] = ResNetDataHandler.tensor_split(
        raw_data=y_output_raw_data, split_dict=y_output_split_dict, data_type=1, alignment=(16, None, None),
        dims=(0, 2, 1))
    data['layer3.5.cut5']['output1'] = ResNetDataHandler.tensor_split(
        raw_data=y_output_raw_data[:, ::2, ::2], split_dict=y_output_split_dict1, data_type=1,
...
if __name__ == '__main__':
    ...
    data = generate_g14_data(handler, size_y=4, size_x=8)
    print(handler.names)
