...
class snn_n_model(nn.Module):
    def __init__(self, layer_size=None):
        ...
        self.w = nn.ParameterList(
            [torch.nn.Parameter((3 * torch.rand(layer_size[ly - 1], layer_size[ly])).int().float() - 1) for ly in
             range(1, self.layer_num)])
        self.b = nn.ParameterList(
            [torch.nn.Parameter((0 * torch.rand(1, layer_size[ly])).int().float()) for ly in range(1, self.layer_num)])
    ...
    def forward(self, inputs):  # inputs : [batchsize, feature_num, length_t]
        ...
        for step in range(time_windows):
            ...
            for ly in range(2, self.layer_num + 1):
                h_mem[ly - 1], h_spike[ly - 1] = neuron_block(h_spike[ly - 2], h_mem[ly - 1], h_spike[ly - 1],
                                                              self.w[ly - 2], self.b[ly - 2])

            h_sumspike[self.layer_num - 1] = h_sumspike[self.layer_num - 1].to(device) + h_spike[self.layer_num - 1].to(
                device)
            # print(h_sumspike[self.layer_num-1].shape)
        return h_sumspike[self.layer_num - 1]
...
output_channels1 = 8
batch_size = 1
output_channels2 = 32 * 8


class HNN_Network(nn.Module):
    def __init__(self):
        super(HNN_Network, self).__init__()

        self.snn = snn_n_model(layer_size=[output_channels2, 512, 512, 10]).to(device)
...
x = torch.rand((1, 32 * 8, 4)) * 255    # 4 - time window
x = x.int()
x = x.float()
...
torch.save(model.state_dict(),
           '../../../../../../SoftwareFiles/Wechat/WeChat Files/wxid_bsum3dnfi3gq22/FileStorage/File/2021-07/算法/parameter_HNN.pkl')
torch.save(inout_dict,
           '../../../../../../SoftwareFiles/Wechat/WeChat Files/wxid_bsum3dnfi3gq22/FileStorage/File/2021-07/算法/parameter_HNN_inout.pkl')
