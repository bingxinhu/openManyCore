...
def gen_1_map_config(phase_en, clock_in_phase, size_x, size_y, cuts: None, data=None, in_data_en=False,
                     out_data_en=False, chip=(0, 0), init_data=None, delay_l4=None, delay_l5=None):
    """
    ...
    # ******** 数据交互 ********
    offset = 4
    # 发送 9， 10， 10， 10 * 16，接收 10240B		10240B		 10240B		 11264B
    if phase_en[0]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            soma1 = None
            length_in = 9 * 16 if core_x == 0 else 10 * 16
            num_in = 11264 // 16 if core_x == 3 else 10240 // 16
            if out_data_en:
                soma1 = p06(addr_in=0xc800 >> 2, addr_out=0x9000, addr_ciso=0x0000 >> 2, length_in=length_in,
                            ...
                            data_in=data['max_pool']['output'][(core_x, core_y)] if init_data else None,
                            data_ciso=None)
            ...
            router = p09(rhead_mode=1, send_en=out_data_en,
                         receive_en=in_data_en, send_num=length_in // 8 - 1, receive_num=0,
                         addr_din_base=0x380, addr_din_length=num_in * 2 - 1, addr_rhead_base=addr_rhead_base,
            ...
            soma2 = None
            if in_data_en:
                soma2 = p06(addr_in=0x8380, addr_out=(0x0000 + 0) >> 2, addr_ciso=0x0000 >> 2, length_in=16,
    ...
    # 接收 10240B		10240B		 10240B		 11264B
    if phase_en[1]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            if core_x == 0 and core_y:
                axon = p41(px=1, py=1, load_bias=0, cin=1, cout=1, kx=1, ky=1, sx=1, sy=1, addr_ina=0x1f000 >> 2,
                        addr_inb=0x1f000 >> 2, addr_bias=0x1f000 >> 2, addr_out=0x1f000 >> 2, axon_delay=True,
            ...
            soma2 = None
            if in_data_en:
                soma2 = p06(addr_in=0x8380, addr_out=(0x0000 + num_in * 16) >> 2, addr_ciso=0x0000 >> 2, length_in=16,
                            length_out=16, length_ciso=1, num_in=num_in, num_ciso=num_in, num_out=num_in, type_in=1,
                            type_out=1, in_cut_start=0, in_row_max=0, row_ck_on=0,
                            data_in=None, data_ciso=None)
            phase_group[(chip, (core_x, core_y))]['prims'].append({'axon': axon, 'soma1': soma1, 'router': router,
    ...
    if phase_en[2]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            soma2 = None
            if in_data_en:
                soma2 = p06(addr_in=0x8380, addr_out=(0x0000 + num_in * 32) >> 2, addr_ciso=0x0000 >> 2, length_in=16,
                            length_out=16, length_ciso=1, num_in=num_in, num_ciso=num_in, num_out=num_in, type_in=1,
                            type_out=1, in_cut_start=0, in_row_max=0, row_ck_on=0,
                            data_in=None, data_ciso=None)
            phase_group[(chip, (core_x, core_y))]['prims'].append({'axon': axon, 'soma1': soma1, 'router': router,
    ...
    # 接收 10400B		 10400B 		 10400B 		 11440B
    if phase_en[3]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            axon = None
            soma1 = None
            ...
            soma2 = None
            if in_data_en:
                soma2 = p06(addr_in=0x8380, addr_out=(0x0000 + last_num_in * 48) >> 2, addr_ciso=0x0000 >> 2,
                            length_in=16,
                            length_out=16, length_ciso=1, num_in=num_in, num_ciso=num_in, num_out=num_in, type_in=1,
                            type_out=1, in_cut_start=0, in_row_max=0, row_ck_on=0,
                            data_in=None, data_ciso=None)
    ...
    if phase_en[offset + 0]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            if core_x in [3]:
                ...
                length = 10
            soma1 = p06(addr_in=0x0000 >> 2, addr_out=0x10980 >> 2, addr_ciso=0x10000 >> 2, length_in=16 * length,
                        length_out=16 * length, length_ciso=1, num_in=257, num_ciso=257, num_out=257, type_in=1,
                        type_out=1, in_cut_start=0, in_row_max=0, row_ck_on=0,
                        data_in=data['conv1']['input'][(core_x, core_y)] if not in_data_en else None,
                        data_ciso=None)
            if in_data_en:
                soma1 = None
            router = None
