...
def check(data: dict, compare: ResultCompare, size_y, size_x, chip=(0, 0), empty_offset=0):
    ...
    for core_y, core_x in product(range(*size_y), range(*size_x)):
        ...
        data_1[empty_offset + 0][(chip, (core_x, core_y))] = [{
            'data': data['layer4.2.conv1']['input1'][(core_x - size_x[0], core_y - size_y[0])],
            'addr_start': 0x19000 >> 2,
            'type': 1
        }]
        # receive 25*512
        data_1[empty_offset + 1][(chip, (core_x, core_y))] = [{
            'data': data['layer4.2.conv1']['input3'][(core_x - size_x[0], core_y - size_y[0])],
            'addr_start': 0x1c000 >> 2,
            'type': 1
        }]
        # L45 in
        data_1[empty_offset + offset + 2 + 1][(chip, (core_x, core_y))] = [{
            'data': data['layer4.2.cut1']['output2'][(core_x - size_x[0], core_y - size_y[0])],
            'addr_start': 0x8300 >> 2,
            'type': 1
        }]
        # shortcut in
        data_1[empty_offset + offset + 3 + 1][(chip, (core_x, core_y))] = [{
            'data': data['layer4.2.conv1']['input2'][(core_x - size_x[0], core_y - size_y[0])],
            'addr_start': 0x1f980 >> 2,
            'type': 1
        }]
        # L46 in
        data_1[empty_offset + offset + 6 + 2][(chip, (core_x, core_y))] = [{
            'data': data['layer4.2.cut2']['output2'][(core_x - size_x[0], core_y - size_y[0])],
            'addr_start': 0x19000 >> 2,
            'type': 1
        }]
        # L46 out 1/3
        data_1[empty_offset + offset + 8 + 2][(chip, (core_x, core_y))] = [{
            'data': data['layer4.2.cut3']['output1'][(core_x - size_x[0], core_y - size_y[0])],
            'addr_start': 0x8300 >> 2,
            'type': 1
        }]
        # L46 out 2/3
        data_1[empty_offset + offset + 10 + 2][(chip, (core_x, core_y))] = [{
            'data': data['layer4.2.cut3']['output2'][(core_x - size_x[0], core_y - size_y[0])],
            'addr_start': 0x8600 >> 2,
            'type': 1
        }]
        # L46 out 3/3
        if core_y == 0:
            data_1[empty_offset + offset + 12 + 2][(chip, (core_x, core_y))] = [{
                'data': data['layer4.2.cut3']['output3'][(core_x - size_x[0], core_y - size_y[0])],
                'addr_start': 0x8900 >> 2,
                'type': 1
            }]
        # # L46 out 12, 12, 12, 13
        # data_1[empty_offset + offset + 13][(chip, (core_x, core_y))] = [{
        #     'data': data['layer4.2.cut3']['output4'][(core_x - size_x[0], core_y - size_y[0])],
        ...
        if core_y == 0:
            ...
            data_1[empty_offset + offset + 14 + 2][(chip, (core_x, core_y))] = [{
                'data': data['layer4.2.cut5']['output2'][(core_x - size_x[0], core_y - size_y[0])],
                'addr_start': 0xe780 >> 2,
                'type': 1
            }]
            # Y AvgPool out
            data_1[empty_offset + offset + 15 + 2][(chip, (core_x, core_y))] = [{
                'data': data['avgpool']['output'][(core_x - size_x[0], core_y - size_y[0])],
...
if __name__ == '__main__':
    ...
    check(data, size_y=4, size_x=16, chip=(1, 1), data_file_name='R00017')
