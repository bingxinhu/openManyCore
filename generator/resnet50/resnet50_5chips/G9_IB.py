...
def gen_g9_ib_map_config(phase_en, clock_in_phase, data=None,
                         in_data_en=False, out_data_en=False, delay_l4=None, delay_l5=None, chip=(0, 0),
                         init_data=None):
    """
        ResNet-50 5-Chip Group-9 Input Buffer
        core array : [(8, 0), (9, 0), (10, 0), (11, 0)]
    """
    map_config = {
        'sim_clock': None,
        (chip, 0): {
    ...
    if phase_en[0]:
        for core_x, core_y in [(8, 0), (9, 0), (10, 0), (11, 0)]:
            axon = p41(px=1, py=1, load_bias=0, cin=1, cout=1, kx=1, ky=1, sx=1, sy=1, addr_ina=0x0 >> 2,
                       addr_inb=0x0 >> 2, addr_bias=0x0 >> 2, addr_out=0x0 >> 2, axon_delay=True,
                       L4_num=delay_l4[0], L5_num=delay_l5[0], A2S2_mode=True)
            # delay clock = 6 + 37 * (L4 + 1) * (L5 + 1)
            if (core_x, core_y) != (8, 0):
                axon = None
            soma1 = None
            if out_data_en:
                soma1 = pX5(mode='max', addr_in=0x19000 >> 2, addr_out=0x9000, cin=512, cout=512, px=4, py=6, kx=1,
                            ky=1, sx=1, sy=1, cmp_c=0x80808080, type_in=1, type_out=1, in_cut_start=0, row_ck_on=0,
                            in_row_max=1,
                            data_in=data['layer3.0.conv1']['input1'][(core_x, core_y)] if init_data else None)
            ...
            router = p09(rhead_mode=1, send_en=out_data_en,
                         receive_en=in_data_en, send_num=24 * 512 // 8 - 1, receive_num=0,
            ...
            if not in_data_en:
                # soma2 = p06(addr_in=0x10000 >> 2, addr_out=0x0000 >> 2, addr_ciso=0, length_in=512, length_out=512,
                #             length_ciso=1, num_in=24, num_ciso=24, num_out=24, type_in=1, type_out=1,
                #             in_cut_start=0, in_row_max=0, row_ck_on=0, data_in=None, data_ciso=None)
                soma2 = None
        ...
        for core_x, core_y in [(8, 0), (9, 0), (10, 0), (11, 0)]:
            ...
            if out_data_en:
                soma1 = pX5(mode='max', addr_in=0x1c000 >> 2, addr_out=0x9000, cin=512, cout=512, px=5, py=5, kx=1,
                            ky=1, sx=1, sy=1, cmp_c=0x80808080, type_in=1, type_out=1, in_cut_start=0, row_ck_on=0,
                            in_row_max=1,
                            data_in=data['layer3.0.conv1']['input2'][(core_x, core_y)] if init_data else None)
            addr_rhead_base = MapConfigGen.get_router_rhead_base(phase_group[(chip, (core_x, core_y))]['prims'],
                                                                 limit=0x380)
            router = p09(rhead_mode=1, send_en=out_data_en,
                         receive_en=in_data_en, send_num=25 * 512 // 8 - 1, receive_num=0,
    ...
    if phase_en[2]:
        for core_x, core_y in [(8, 0), (9, 0), (10, 0), (11, 0)]:
            ...
            soma1 = None
            if in_data_en:
                soma1 = pX5(mode='max', addr_in=0x10000 >> 2, addr_out=0x19000 >> 2, cin=512, cout=512, px=7, py=7,
                            kx=1, ky=1, sx=1, sy=1, cmp_c=0x80808080, type_in=1, type_out=1, in_cut_start=0,
                            row_ck_on=0, in_row_max=1, data_in=None)
            router = None
            soma2 = None
            phase_group[(chip, (core_x, core_y))]['prims'].append({'axon': axon, 'soma1': soma1, 'router': router,
                                                                   'soma2': soma2})
