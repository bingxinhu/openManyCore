...
def generate_g1_data(handler, size_y, size_x):
    ...
    # FCa2 input
    fca2_input_raw_data = handler.parameters['fc']['input'].astype(np.int8)
    fca2_input_split_dict = {}
    for core_y, core_x in product(range(size_y), range(size_x)):
        fca2_input_split_dict[(core_x, core_y)] = ((0, fca2_input_raw_data.shape[0]),)

    g1_data['fca2_input'] = ResNetDataHandler.tensor_split(raw_data=fca2_input_raw_data,
                                                           split_dict=fca2_input_split_dict,
                                                           data_type=1, alignment=(32,), dims=[0])

    # FCa2 weight
    fca2_weight_raw_data = handler.parameters['fc']['weight'].astype(np.int8)
    fca2_weight_split_dict = {}
    for core_y, core_x in product(range(size_y), range(size_x)):
        if core_x == size_x - 1 and core_y == size_y - 1:
            fca2_weight_split_dict[(core_x, core_y)] = (((core_x + core_y * size_x) * 32,
                                                         fca2_weight_raw_data.shape[0]),
                                                        (0, fca2_weight_raw_data.shape[1]))
        else:
            fca2_weight_split_dict[(core_x, core_y)] = (((core_x + core_y * size_x) * 32,
                                                         (core_x + core_y * size_x + 1) * 32),
                                                        (0, fca2_weight_raw_data.shape[1]))

    g1_data['fca2_weight'] = ResNetDataHandler.tensor_split(raw_data=fca2_weight_raw_data,
                                                            split_dict=fca2_weight_split_dict,
                                                            data_type=1, alignment=(32, None), dims=[0, 1],
                                                            is_weight=True)

    # FCa2 bias
    fca2_bias_raw_data = handler.parameters['fc']['bias'].astype(np.int32)
    fca2_bias_split_dict = {}
    for core_y, core_x in product(range(size_y), range(size_x)):
        if core_x == size_x - 1 and core_y == size_y - 1:
            fca2_bias_split_dict[(core_x, core_y)] = (((core_x + core_y * size_x) * 32,
                                                       fca2_bias_raw_data.shape[0]),)
        else:
            fca2_bias_split_dict[(core_x, core_y)] = (((core_x + core_y * size_x) * 32,
                                                       (core_x + core_y * size_x + 1) * 32),)

    g1_data['fca2_bias'] = ResNetDataHandler.tensor_split(raw_data=fca2_bias_raw_data,
                                                          split_dict=fca2_bias_split_dict,
                                                          data_type=0, alignment=(32,), dims=[0])

    # FCa2 output
    fca2_output_raw_data = handler.parameters[list(handler.names)[-1]]['output'].astype(np.int8)
