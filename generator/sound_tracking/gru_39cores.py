...
def gen_gru_map_config(phase_en, clock_in_phase, size_x, size_y, in_cut_start_mat=None, data=None, handler=None,
    ...
    if phase_en[0]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            if core_id < handler.sequence_length:
                ...
                soma1 = p06(addr_in=0x0000 >> 2, addr_out=0x24000 >> 2, addr_ciso=0xfee0 >> 2, length_in=39 * 32,
                            num_in=1, length_ciso=256 + 32, num_ciso=1, length_out=1536, num_out=1, type_in=1,
                            type_out=1,
                ...
                router = p09(rhead_mode=1, send_en=out_data_en, receive_en=in_data_en, send_num=192 - 1,
                             receive_num=3 if core_id == 0 else 0, addr_din_base=0x1000 >> 2,
                             addr_din_length=156 - 1 if core_id == 0 else 192 - 1, addr_rhead_base=addr_rhead_base,
                ...
                if 0 <= core_id < 15 or core_id >= 32:
                    ...
                    X = -1
                    Y = 0
                router.addRHead(S=0, T=1, P=0, Q=0, X=X, Y=Y, A=0, pack_per_Rhead=192 - 1, A_offset=0, Const=0, EN=1)
                soma2 = p26(addr_in=0x21000 >> 2, addr_out=0x0000 >> 2, addr_ciso=0x1fee0 >> 2,
                            length_in=39 * 32 if core_id == 0 else 1536,
                            num_in=1, length_ciso=256 + 32, num_ciso=1, length_out=39 * 32, num_out=1, type_in=1,
                            type_out=1)
    ...
    if phase_en[offset + 0]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            if core_id < handler.sequence_length:
                ...
                axon = p04(cin=17, cout=128, addr_ina=(0x0000 + core_id * 0x0020) >> 2, addr_inb=0x10000 >> 2,
                           addr_bias=0x10800 >> 2,
                           addr_out=(0x0280 + 0x260) >> 2, ina_type=1, inb_type=1, load_bias=0, bias_length=128,
                ...
                router = None
                # pad '127' for Ht-1
                soma2 = p26(addr_in=0x1e600 >> 2, addr_out=0x1ff60 >> 2, addr_ciso=0x1fff0 >> 2,
                            length_in=16 * 2, num_in=1, length_ciso=16, num_ciso=1, length_out=16, num_out=1, type_in=1,
                            type_out=1, data_in=data['pad_127_for_input'])
                phase_group[(chip, (core_x, core_y))]['prims'].append({'axon': axon, 'soma1': soma1, 'router': router,
                                                                       'soma2': soma2})
    ...
    if phase_en[offset + 1]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            if core_id < handler.sequence_length:
                ...
                axon = p04(cin=129, cout=128, addr_ina=0x1fee0 >> 2, addr_inb=0x10a00 >> 2, addr_bias=0x14a00 >> 2,
                           addr_out=(0x0480 + 0x260) >> 2, ina_type=1, inb_type=1, load_bias=0,
                           data_x=data_x, data_w=data_w, data_b=None)
                axon = p41(px=1, py=1, cin=axon.cin, cout=axon.cout, kx=1, ky=1, sx=1, sy=1,
                           addr_ina=axon.Addr_InA_base, addr_inb=axon.Addr_InB_base, addr_bias=axon.Addr_Bias_base,
                           addr_out=axon.Addr_V_base, ina_type=axon.InA_type, inb_type=axon.InB_type,
        ...
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            if core_id < handler.sequence_length:
                ...
                axon = p02(avg_pooling_en=False, ina_type=0, load_bias=0, kx=1, ky=2, sx=1, sy=1, cin=128, px=1, py=1,
                           addr_in=(0x0280 + 0x260) >> 2, addr_bias=0x0, addr_out=(0x0680 + 0x260) >> 2, pad_top=0,
                           pad_down=0, pad_left=0,
                           pad_right=0, bias_length=0, data_x=None, data_b=None, constant_b=0)
                soma1 = p07(addr_in=(0x0680 + 0x260) >> 2, addr_out=(0x0880 + 0x260) >> 2, addr_lut=0x14c00 >> 2,
                            group_num=1,
                            neuron_num=128,
    ...
    if phase_en[offset + 3]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            if core_id < handler.sequence_length:
                ...
                axon = p04(cin=17, cout=128, addr_ina=(0x0000 + core_id * 0x0020) >> 2, addr_inb=0x14d00 >> 2,
                           addr_bias=0x15500 >> 2,
                           addr_out=(0x0280 + 0x260) >> 2, ina_type=1, inb_type=1, load_bias=0,
                           data_x=None, data_w=data_w, data_b=None)
                axon = p41(px=1, py=1, cin=axon.cin, cout=axon.cout, kx=1, ky=1, sx=1, sy=1,
                           addr_ina=axon.Addr_InA_base, addr_inb=axon.Addr_InB_base, addr_bias=axon.Addr_Bias_base,
                           addr_out=axon.Addr_V_base, ina_type=axon.InA_type, inb_type=axon.InB_type,
                           load_bias=axon.Load_Bias, pad_top=0, pad_down=0, pad_left=0, pad_right=0,
    ...
    if phase_en[offset + 4]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            if core_id < handler.sequence_length:
                ...
                axon = p04(cin=129, cout=128, addr_ina=0x1fee0 >> 2, addr_inb=0x15700 >> 2, addr_bias=0x19700 >> 2,
                           addr_out=(0x0480 + 0x260) >> 2, ina_type=1, inb_type=1, load_bias=0,
                           data_x=None, data_w=data_w, data_b=None)
                axon = p41(px=1, py=1, cin=axon.cin, cout=axon.cout, kx=1, ky=1, sx=1, sy=1,
                           addr_ina=axon.Addr_InA_base, addr_inb=axon.Addr_InB_base, addr_bias=axon.Addr_Bias_base,
        ...
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            if core_id < handler.sequence_length:
                axon = p02(avg_pooling_en=False, ina_type=0, load_bias=0, kx=1, ky=2, sx=1, sy=1, cin=128, px=1, py=1,
                           ...
                           pad_down=0, pad_left=0,
                           pad_right=0, bias_length=0, data_x=None, data_b=None, constant_b=0)
                soma1 = p07(addr_in=(0x0680 + 0x260) >> 2, addr_out=(0x0900 + 0x260) >> 2, addr_lut=0x14c00 >> 2, group_num=1,
                            neuron_num=128,
                            lut_dw=1, type_in=0, type_out=1,
    ...
    if phase_en[offset + 6]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            if core_id < handler.sequence_length:
                ...
                axon = p04(cin=17, cout=128, addr_ina=(0x0000 + core_id * 0x0020) >> 2, addr_inb=0x19900 >> 2,
                           addr_bias=0x1a100 >> 2,
                           addr_out=(0x0280 + 0x260) >> 2, ina_type=1, inb_type=1, load_bias=0,
                           data_x=None, data_w=data_w, data_b=None)
                axon = p41(px=1, py=1, cin=axon.cin, cout=axon.cout, kx=1, ky=1, sx=1, sy=1,
    ...
    if phase_en[offset + 7]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            if core_id < handler.sequence_length:
                ...
                axon = p04(cin=129, cout=128, addr_ina=0x1fee0 >> 2, addr_inb=0x1a300 >> 2, addr_bias=0x1e300 >> 2,
                           addr_out=(0x0480 + 0x260) >> 2, ina_type=1, inb_type=1, load_bias=0,
                           data_x=None, data_w=data_w, data_b=None)
                axon = p41(px=1, py=1, cin=axon.cin, cout=axon.cout, kx=1, ky=1, sx=1, sy=1,
                           addr_ina=axon.Addr_InA_base, addr_inb=axon.Addr_InB_base, addr_bias=axon.Addr_Bias_base,
                ...
                soma1 = None
                router = None
                soma2 = pX5(mode='max', addr_in=(0x0480 + 0x260) >> 2, addr_out=(0x0980 + 0x260) >> 2, cin=128,
                            cout=128, px=1, py=1, kx=1,
        ...
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            if core_id < handler.sequence_length:
                axon = p03(tensor_en=False, x_array_num=1, cin=128, tensor_px=1, tensor_py=1, tensor_sx=1, tensor_sy=1,
                           addr_ina=(0x0880 + 0x260) >> 2, addr_inb=(0x0980 + 0x260) >> 2, addr_bias=0x0000 >> 2,
                           addr_out=(0x0480 + 0x260) >> 2, ina_type=1, load_bias=0, bias_length=0,
        ...
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            if core_id < handler.sequence_length:
                ...
                axon = p02(avg_pooling_en=False, ina_type=0, load_bias=0, kx=1, ky=2, sx=1, sy=1, cin=128, px=1, py=1,
                           ...
                           pad_down=0, pad_left=0,
                           pad_right=0, bias_length=0, data_x=None, data_b=None, constant_b=0)
                soma1 = p07(addr_in=(0x0680 + 0x260) >> 2, addr_out=(0x0880 + 0x260) >> 2, addr_lut=0x1e500 >> 2,
                            group_num=1,
        ...
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            if core_id < handler.sequence_length:
                axon = p83(tensor_en=False, x_array_num=1, cin=128, tensor_px=1, tensor_py=1, tensor_sx=1, tensor_sy=1,
                           addr_ina=(0x0900 + 0x260) >> 2, addr_bias=0x0000 >> 2, addr_out=(0x0280 + 0x260) >> 2, constant_a=-1,
                           constant_b=q_ones[core_id]['forward'], ina_type=1, load_bias=0, bias_length=0,
                           data_x=None, data_b=None)
                soma1 = pX5(mode='max', addr_in=(0x0280 + 0x260) >> 2, addr_out=(0x0980 + 0x260) >> 2, cin=128,
                            cout=128, px=1, py=1, kx=1,
                            ky=1, sx=1, sy=1, cmp_c=0x80808080, type_in=0, type_out=1,
    ...
    if phase_en[offset + 11]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            if core_id < handler.sequence_length:
                axon = p03(tensor_en=False, x_array_num=1, cin=128, tensor_px=1, tensor_py=1, tensor_sx=1, tensor_sy=1,
                           addr_ina=(0x0880 + 0x260) >> 2, addr_inb=(0x0980 + 0x260) >> 2, addr_bias=0x0000 >> 2,
                           addr_out=(0x0280 + 0x260) >> 2, ina_type=1, load_bias=0, bias_length=0,
                           data_x=None, data_y=None, data_b=None)
                soma1 = None
                router = None
                soma2 = None
    ...
    if phase_en[offset + 12]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            if core_id < handler.sequence_length:
                axon = p03(tensor_en=False, x_array_num=1, cin=128, tensor_px=1, tensor_py=1, tensor_sx=1, tensor_sy=1,
                           addr_ina=(0x0900 + 0x260) >> 2, addr_inb=0x1fee0 >> 2, addr_bias=0x0000 >> 2,
                           addr_out=(0x0480 + 0x260) >> 2, ina_type=1, load_bias=0, bias_length=0,
                           data_x=None, data_y=None, data_b=None)
                soma1 = None
                router = None
        ...
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            if core_id < handler.sequence_length:
                axon = p02(avg_pooling_en=False, ina_type=0, load_bias=0, kx=1, ky=2, sx=1, sy=1, cin=128, px=1, py=1,
                           ...
                           pad_down=0, pad_left=0,
                           pad_right=0, bias_length=0, data_x=None, data_b=None, constant_b=0)
                soma1 = pX5(mode='max', addr_in=(0x0680 + 0x260) >> 2, addr_out=0xfee0 >> 2, cin=128, cout=128, px=1,
                            py=1, kx=1,
                            ky=1, sx=1, sy=1, cmp_c=0x80808080, type_in=0, type_out=1,
    ...
    if phase_en[offset + 14]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            if core_id < handler.sequence_length:
                ...
                axon = p04(cin=17, cout=128, addr_ina=(0x04c0 - core_id * 0x0020) >> 2,
                           addr_inb=(0x10000 - 0xe800) >> 2, addr_bias=(0x10800 - 0xe800) >> 2,
                           addr_out=(0x0280 + 0x260) >> 2, ina_type=1, inb_type=1, load_bias=0, bias_length=128,
                           data_x=data_x, data_w=data_w, data_b=None)
                axon = p41(px=1, py=1, cin=axon.cin, cout=axon.cout, kx=1, ky=1, sx=1, sy=1,
                           addr_ina=axon.Addr_InA_base, addr_inb=axon.Addr_InB_base, addr_bias=axon.Addr_Bias_base,
                ...
                router = None
                # 让起始地址32B对齐
                soma2 = p06(addr_in=0x1ff70 >> 2, addr_out=0x1ff60 >> 2, addr_ciso=0x0000 >> 2, length_in=128 + 16,
                            num_in=1, length_ciso=1, num_ciso=1, length_out=128 + 16, num_out=1, type_in=1,
    ...
    if phase_en[offset + 15]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            if core_id < handler.sequence_length:
                ...
                axon = p04(cin=129, cout=128, addr_ina=0x1ff60 >> 2, addr_inb=(0x10a00 - 0xe800) >> 2,
                           addr_bias=(0x14a00 - 0xe800) >> 2,
                           addr_out=(0x0480 + 0x260) >> 2, ina_type=1, inb_type=1, load_bias=0,
                           data_x=data_x, data_w=data_w, data_b=None)
                axon = p41(px=1, py=1, cin=axon.cin, cout=axon.cout, kx=1, ky=1, sx=1, sy=1,
                           addr_ina=axon.Addr_InA_base, addr_inb=axon.Addr_InB_base, addr_bias=axon.Addr_Bias_base,
        ...
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            if core_id < handler.sequence_length:
                ...
                axon = p02(avg_pooling_en=False, ina_type=0, load_bias=0, kx=1, ky=2, sx=1, sy=1, cin=128, px=1, py=1,
                           ...
                           pad_down=0, pad_left=0,
                           pad_right=0, bias_length=0, data_x=None, data_b=None, constant_b=0)
                soma1 = p07(addr_in=(0x0680 + 0x260) >> 2, addr_out=(0x0880 + 0x260) >> 2,
                            addr_lut=(0x14c00 - 0xe800) >> 2,
    ...
    if phase_en[offset + 17]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            if core_id < handler.sequence_length:
                ...
                axon = p04(cin=17, cout=128, addr_ina=(0x04c0 - core_id * 0x0020) >> 2,
                           addr_inb=(0x14d00 - 0xe800) >> 2, addr_bias=(0x15500 - 0xe800) >> 2,
                           addr_out=(0x0280 + 0x260) >> 2, ina_type=1, inb_type=1, load_bias=0,
                           data_x=None, data_w=data_w, data_b=None)
                axon = p41(px=1, py=1, cin=axon.cin, cout=axon.cout, kx=1, ky=1, sx=1, sy=1,
                           addr_ina=axon.Addr_InA_base, addr_inb=axon.Addr_InB_base, addr_bias=axon.Addr_Bias_base,
    ...
    if phase_en[offset + 18]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            if core_id < handler.sequence_length:
                ...
                axon = p04(cin=129, cout=128, addr_ina=0x1ff60 >> 2, addr_inb=(0x15700 - 0xe800) >> 2,
                           addr_bias=(0x19700 - 0xe800) >> 2,
                           addr_out=(0x0480 + 0x260) >> 2, ina_type=1, inb_type=1, load_bias=0,
                           data_x=None, data_w=data_w, data_b=None)
                axon = p41(px=1, py=1, cin=axon.cin, cout=axon.cout, kx=1, ky=1, sx=1, sy=1,
                           addr_ina=axon.Addr_InA_base, addr_inb=axon.Addr_InB_base, addr_bias=axon.Addr_Bias_base,
        ...
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            if core_id < handler.sequence_length:
                axon = p02(avg_pooling_en=False, ina_type=0, load_bias=0, kx=1, ky=2, sx=1, sy=1, cin=128, px=1, py=1,
                           ...
                           pad_down=0, pad_left=0,
                           pad_right=0, bias_length=0, data_x=None, data_b=None, constant_b=0)
                soma1 = p07(addr_in=(0x0680 + 0x260) >> 2, addr_out=(0x0900 + 0x260) >> 2, addr_lut=(0x14c00 - 0xe800) >> 2,
                            group_num=1,
                            neuron_num=128,
                            lut_dw=1, type_in=0, type_out=1,
    ...
    if phase_en[offset + 20]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            if core_id < handler.sequence_length:
                ...
                axon = p04(cin=17, cout=128, addr_ina=(0x04c0 - core_id * 0x0020) >> 2,
                           addr_inb=(0x19900 - 0xe800) >> 2, addr_bias=(0x1a100 - 0xe800) >> 2,
                           addr_out=(0x0280 + 0x260) >> 2, ina_type=1, inb_type=1, load_bias=0,
                           data_x=None, data_w=data_w, data_b=None)
                axon = p41(px=1, py=1, cin=axon.cin, cout=axon.cout, kx=1, ky=1, sx=1, sy=1,
                           addr_ina=axon.Addr_InA_base, addr_inb=axon.Addr_InB_base, addr_bias=axon.Addr_Bias_base,
    ...
    if phase_en[offset + 21]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            if core_id < handler.sequence_length:
                ...
                axon = p04(cin=129, cout=128, addr_ina=0x1ff60 >> 2, addr_inb=(0x1a300 - 0xe800) >> 2,
                           addr_bias=(0x1e300 - 0xe800) >> 2,
                           addr_out=(0x0480 + 0x260) >> 2, ina_type=1, inb_type=1, load_bias=0,
                           data_x=None, data_w=data_w, data_b=None)
                axon = p41(px=1, py=1, cin=axon.cin, cout=axon.cout, kx=1, ky=1, sx=1, sy=1,
                           addr_ina=axon.Addr_InA_base, addr_inb=axon.Addr_InB_base, addr_bias=axon.Addr_Bias_base,
        ...
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            if core_id < handler.sequence_length:
                ...
                axon = p02(avg_pooling_en=False, ina_type=0, load_bias=0, kx=1, ky=2, sx=1, sy=1, cin=128, px=1, py=1,
                           ...
                           pad_right=0, bias_length=0, data_x=None, data_b=None, constant_b=0)
                soma1 = p07(addr_in=(0x0680 + 0x260) >> 2, addr_out=(0x0880 + 0x260) >> 2,
                            addr_lut=(0x1e500 - 0xe800) >> 2, group_num=1,
                            neuron_num=128, lut_dw=1, type_in=0, type_out=1,
    ...
    if phase_en[offset + 26]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            if core_id < handler.sequence_length:
                axon = p03(tensor_en=False, x_array_num=1, cin=128, tensor_px=1, tensor_py=1, tensor_sx=1, tensor_sy=1,
                           addr_ina=(0x0900 + 0x260) >> 2, addr_inb=0x1ff60 >> 2, addr_bias=0x0000 >> 2,
                           addr_out=(0x0480 + 0x260) >> 2, ina_type=1, load_bias=0, bias_length=0,
                           data_x=None, data_y=None, data_b=None)
                soma1 = None
                router = None
        ...
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            if core_id < handler.sequence_length:
                axon = p02(avg_pooling_en=False, ina_type=0, load_bias=0, kx=1, ky=2, sx=1, sy=1, cin=128, px=1, py=1,
                           ...
                           pad_down=0, pad_left=0,
                           pad_right=0, bias_length=0, data_x=None, data_b=None, constant_b=0)
                soma1 = pX5(mode='max', addr_in=(0x0680 + 0x260) >> 2, addr_out=0xff70 >> 2, cin=128, cout=128, px=1,
                            py=1, kx=1,
                            ky=1, sx=1, sy=1, cmp_c=0x80808080, type_in=0, type_out=1,
                ...
                router = None
                # pad '127' for Ht
                soma2 = p26(addr_in=0x1e600 >> 2, addr_out=0xff60 >> 2, addr_ciso=0xfff0 >> 2,
                            length_in=16 * 2, num_in=1, length_ciso=16, num_ciso=1, length_out=16, num_out=1, type_in=1,
                            type_out=1, data_in=None)
                phase_group[(chip, (core_x, core_y))]['prims'].append({'axon': axon, 'soma1': soma1, 'router': router,
                                                                       'soma2': soma2})
