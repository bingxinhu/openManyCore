...
def gen_g3_map_config(phase_en, clock_in_phase, size_x, size_y, cuts, data=None,
    ...
    if data is None:
        ...
        for core_y, core_x in product(range(size_y), range(size_x)):
            data['layer1.1.conv1']['input'][(core_x, core_y)] = []
            data['layer1.1.conv1']['weight'][(core_x, core_y)] = []
            data['layer1.1.conv1']['bias'][(core_x, core_y)] = []
            data['layer1.1.conv2']['input'][(core_x, core_y)] = []
            data['layer1.1.conv2']['weight'][(core_x, core_y)] = []
            data['layer1.1.conv2']['bias'][(core_x, core_y)] = []
            data['layer1.1.conv3']['input'][(core_x, core_y)] = []
            data['layer1.1.conv3']['weight'][(core_x, core_y)] = []
            data['layer1.1.conv3']['bias'][(core_x, core_y)] = []
        ...
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            if out_data_en:
                soma1 = p06(addr_in=0x9000 >> 2, addr_out=0x9000, addr_ciso=0x0, length_in=256,
                            ...
                            data_in=data['layer1.1.cut5']['output1'][(core_x, core_y)] if init_data else None,
                            data_ciso=None)
            ...
            router = p09(rhead_mode=1, send_en=out_data_en,
                         receive_en=in_data_en, send_num=895, receive_num=3,
    ...
    if phase_en[1]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            soma1 = None
            if out_data_en:
                soma1 = p06(addr_in=0xac00 >> 2, addr_out=0x9000, addr_ciso=0x0, length_in=256,
                            length_out=256, length_ciso=256, num_in=28, num_ciso=28, num_out=28, type_in=1,
                            type_out=1, in_cut_start=0, in_row_max=0, row_ck_on=0,
                            data_in=data['layer1.1.cut5']['output2'][(core_x, core_y)] if init_data else None,
                            data_ciso=None)
            addr_rhead_base = MapConfigGen.get_router_rhead_base(phase_group[(chip, (core_x, core_y))]['prims'],
                                                                 limit=0x380)
            router = p09(rhead_mode=1, send_en=out_data_en,
    ...
    if phase_en[2]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            soma1 = None
            if out_data_en:
                soma1 = p06(addr_in=0xc800 >> 2, addr_out=0x9000, addr_ciso=0x0, length_in=256,
                            length_out=256, length_ciso=256, num_in=28, num_ciso=28, num_out=28, type_in=1,
                            type_out=1, in_cut_start=0, in_row_max=0, row_ck_on=0,
                            data_in=data['layer1.1.cut5']['output3'][(core_x, core_y)] if init_data else None,
                            data_ciso=None)
            addr_rhead_base = MapConfigGen.get_router_rhead_base(phase_group[(chip, (core_x, core_y))]['prims'],
                                                                 limit=0x380)
            router = p09(rhead_mode=1, send_en=out_data_en,
    ...
    if phase_en[3]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            soma1 = None
            if out_data_en:
                soma1 = p06(addr_in=0xe400 >> 2, addr_out=0x9000, addr_ciso=0x0, length_in=256,
                            length_out=256, length_ciso=256, num_in=28, num_ciso=28, num_out=28, type_in=1,
                            type_out=1, in_cut_start=0, in_row_max=0, row_ck_on=0,
                            data_in=data['layer1.1.cut5']['output4'][(core_x, core_y)] if init_data else None,
                            data_ciso=None)
            addr_rhead_base = MapConfigGen.get_router_rhead_base(phase_group[(chip, (core_x, core_y))]['prims'],
                                                                 limit=0x380)
            router = p09(rhead_mode=1, send_en=out_data_en,
    ...
    if phase_en[offset + 0]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            axon = p41(px=28, py=4, cin=256, cout=64, kx=1, ky=1, sx=1, sy=1, addr_ina=0x2000 >> 2,
                       ...
                       data_x=None if in_data_en else data['layer1.1.conv1']['input'][(core_x, core_y)],
                       data_w=data['layer1.1.conv1']['weight'][(core_x, core_y)],
                       data_b=data['layer1.1.conv1']['bias'][(core_x, core_y)])
            ...
            soma1 = pX5(mode='max', addr_in=0x18b80 >> 2, addr_out=addr_out, cin=64, cout=64, px=28, py=4, kx=1,
                        ky=1, sx=1, sy=1, cmp_c=0x0, type_in=0, type_out=1, in_cut_start=cuts['layer1.1.cut1'],
    ...
    if phase_en[offset + 3]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            axon = None
            soma1 = p06(addr_in=0x9e00 >> 2, addr_out=0x9000, addr_ciso=0x0000 >> 2, length_in=64, length_out=64,
                        length_ciso=1, num_in=56, num_ciso=56, num_out=56, type_in=1, type_out=1, in_cut_start=0,
                        in_row_max=0, row_ck_on=0, data_in=None, data_ciso=None)
            addr_rhead_base = MapConfigGen.get_router_rhead_base(phase_group[(chip, (core_x, core_y))]['prims'])
            ...
            router.addRHead(S=0, T=1, P=0, Q=0, X=-1, Y=0, A=0, pack_per_Rhead=447, A_offset=0, Const=0, EN=1)
            soma2 = p06(addr_in=0x8400, addr_out=0xd600 >> 2, addr_ciso=0x0000, length_in=64, length_out=64,
                        length_ciso=1, num_in=56, num_ciso=56, num_out=56, type_in=1, type_out=1,
                        in_cut_start=0, in_row_max=0, row_ck_on=0, data_in=None, data_ciso=None)
            if core_x == 0:
                soma1 = None
    ...
    if phase_en[offset + 4]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            axon = p41(px=56, py=py, cin=64, cout=32, kx=3, ky=3, sx=1, sy=1, addr_ina=addr_ina,
                       ...
                       data_x=None, data_w=data['layer1.1.conv2']['weight'][(core_x, core_y)],
                       data_b=data['layer1.1.conv2']['bias'][(core_x, core_y)])
            soma1 = pX5(mode='max', addr_in=0x18b80 >> 2, addr_out=0x1e400 >> 2, cin=32, cout=32, px=56, py=4, kx=1,
                        ky=1, sx=1, sy=1, cmp_c=0x0, type_in=0, type_out=1, in_cut_start=cuts['layer1.1.cut2'],
                        row_ck_on=1,
                        in_row_max=2)
            router = None
            soma2 = None
    ...
    if phase_en[offset + 5]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            axon = None
            soma1 = p06(addr_in=0x1e400 >> 2, addr_out=0x9000, addr_ciso=0x0000, length_in=56 * 32, length_out=56 * 32,
    ...
    if phase_en[offset + 6]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            axon = p41(px=4, py=56, cin=64, cout=128, kx=1, ky=1, sx=1, sy=1, addr_ina=0x18b80 >> 2,
                       ...
                       data_w=data['layer1.1.conv3']['weight'][(core_x, core_y)],
                       data_b=data['layer1.1.conv3']['bias'][(core_x, core_y)])
            soma1 = pX5(mode='max', addr_in=0x1c380 >> 2, addr_out=0x9000 >> 2, cin=128, cout=128, px=4, py=56, kx=1,
                        ky=1, sx=1, sy=1, cmp_c=0x80808080, type_in=0, type_out=1, in_cut_start=cuts['layer1.1.cut3'],
                        row_ck_on=1, in_row_max=4)
            router = None
            soma2 = None
            phase_group[(chip, (core_x, core_y))]['prims'].append({'axon': axon, 'soma1': soma1, 'router': router,
                                                                   'soma2': soma2})
    # 数据整理 -- 上下互发1*56*128， soma2按通道merge
    if phase_en[offset + 7]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            axon = None
            addr_in = 0xc800 >> 2 if core_y == 0 else 0x9000 >> 2
            ...
            if core_y == 0:
                ...
                addr_in, addr_ciso = 0x8400, 0xc800 >> 2
            soma2 = p06(addr_in=addr_in, addr_out=0x18b80 >> 2, addr_ciso=addr_ciso, length_in=128, length_out=256,
                        length_ciso=128, num_in=56, num_ciso=56, num_out=56, type_in=1, type_out=1,
                        in_cut_start=0, in_row_max=0, row_ck_on=0, data_in=None, data_ciso=None)
            phase_group[(chip, (core_x, core_y))]['prims'].append({'axon': axon, 'soma1': soma1, 'router': router,
                                                                   'soma2': soma2})
    # 数据整理 -- 上下互发1*56*128， soma2按通道merge
    if phase_en[offset + 8]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            axon = None
            addr_in = 0xe400 >> 2 if core_y == 0 else 0xac00 >> 2
            soma1 = p06(addr_in=addr_in, addr_out=0x9000, addr_ciso=0x0000, length_in=128, length_out=128,
                        length_ciso=1, num_in=56, num_ciso=56, num_out=56, type_in=1, type_out=1, in_cut_start=0,
                        in_row_max=0, row_ck_on=0, data_in=None, data_ciso=None)
            addr_rhead_base = MapConfigGen.get_router_rhead_base(phase_group[(chip, (core_x, core_y))]['prims'])
            ...
            router.addRHead(S=0, T=1, P=0, Q=0, X=0, Y=1 if core_y == 0 else -1,
                            A=0, pack_per_Rhead=895, A_offset=0, Const=0, EN=1)
            if core_y == 0:
                addr_in, addr_ciso = 0xac00 >> 2, 0x8400
            else:
                addr_in, addr_ciso = 0x8400, 0xe400 >> 2
            soma2 = p06(addr_in=addr_in, addr_out=0x1c380 >> 2, addr_ciso=addr_ciso, length_in=128, length_out=256,
                        length_ciso=128, num_in=56, num_ciso=56, num_out=56, type_in=1, type_out=1,
                        in_cut_start=0, in_row_max=0, row_ck_on=0, data_in=None, data_ciso=None)
            phase_group[(chip, (core_x, core_y))]['prims'].append({'axon': axon, 'soma1': soma1, 'router': router,
                                                                   'soma2': soma2})
    # 将整理后的数据从Mem1搬运到Mem0 2 * 56 * 256
    if phase_en[offset + 9]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            axon = None
            soma1 = p06(addr_in=0x18b80 >> 2, addr_out=0x9000 >> 2, addr_ciso=0x0000, length_in=2 * 256,
                        length_out=2 * 256, length_ciso=1, num_in=56, num_ciso=56, num_out=56, type_in=1, type_out=1,
                        in_cut_start=0, in_row_max=0, row_ck_on=0, data_in=None, data_ciso=None)
            router = None
            soma2 = None
            phase_group[(chip, (core_x, core_y))]['prims'].append({'axon': axon, 'soma1': soma1, 'router': router,
    ...
    if phase_en[offset + 10]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            axon = p02(avg_pooling_en=False, ina_type=1, load_bias=0, kx=1, ky=2, sx=1, sy=1, cin=256, px=4, py=28,
                       addr_in=0x2000 >> 2, addr_bias=0x0, addr_out=0x18b80 >> 2, pad_top=0, pad_down=0, pad_left=0,
                       pad_right=0, bias_length=0, data_x=None, data_b=None, constant_b=0)
            soma1 = pX5(mode='max', addr_in=0x18b80 >> 2, addr_out=0x9000 >> 2, cin=256, cout=256, px=4, py=28,
                        kx=1, ky=1, sx=1, sy=1, cmp_c=0x0, type_in=0, type_out=1, in_cut_start=cuts['layer1.1.cut5'],
                        row_ck_on=1, in_row_max=4)
            router = None
            soma2 = None
            phase_group[(chip, (core_x, core_y))]['prims'].append({'axon': axon, 'soma1': soma1, 'router': router,
...
if __name__ == '__main__':
    ...
    phase[4] = 1  # L6 卷积，流水ReLU
    phase[5] = 1  # 整理数据 -- 上下互发送4*56*32的数据

    phase[6] = 1  # L4 卷积，流水截取
    phase[7] = 1  # 数据整理 -- 上下互发1*56*128， soma2按通道merge
    phase[8] = 1  # 数据整理 -- 上下互发1*56*128， soma2按通道merge
    phase[9] = 1  # 将整理后的数据从Mem1搬运到Mem0
    phase[10] = 1  # 求和，流水ReLU

    from generator.resnet50.resnet50_5chips.G3_data import generate_g3_data

    handler = ResNetDataHandler()
    data = generate_g3_data(handler, size_y=2, size_x=14)

    clock_in_phase = 50_000
    config = gen_g3_map_config(phase, clock_in_phase=clock_in_phase, size_x=14, size_y=2, data=data, in_data_en=False,
