...
from generator.test.SCH_Mapping.G9_16cores_2_8_instant import Gen_G9_shortcut_Map_Config
from generator.test.SCH_Mapping.G9_instant import Gen_G9_instant
from primitive import Prim_41_Axon, Prim_X5_Soma, Prim_09_Router, Prim_06_move_merge

def Gen_G9_Map_Config(phase_en, clock, M, N, start_row=0, in_data_en=0, out_data_en=0, delay_L4=(0, 0, 0, 0, 0, 0), delay_L5=(0, 0, 0, 0, 0, 0)):
    """
    ...
    #     core_x = core % M
    #     core_y = core // M
    #     final_map_config[0][0][((0, 0), (core_x, core_y))] = {
    #         'axon': [],
    #         'soma1': [],
    #         'router': [],
    #         'soma2': []
        ...
        for core in range(start_row * M, start_row * M + core_num):
            ...
            if in_data_en == 1:
                soma2 = pX5(core_x=core_x, core_y=core_y, PIC_mode=0, pad_on=False, type_in=1, type_out=1, cin=128, \
                            ...
                            in_cut_start=0, row_ck_on=0, addr_in=0x8380, addr_out=(0x10000+20*128)>>2)
                map_config[0][0][((0, 0), (core_x, core_y))]['soma2'].append(soma2)
            else:
                map_config[0][0][((0, 0), (core_x, core_y))]['soma2'].append(None)
    ...
    if phase_en[2] == 1:
        ...
        for core in range(start_row * M, start_row * M + core_num):
            ...
            if in_data_en == 1:
                soma2 = pX5(core_x=core_x, core_y=core_y, PIC_mode=0, pad_on=False, type_in=1, type_out=1, cin=128, \
                            cout=128, px=10, py=2, kx=1, ky=1, sx=1, sy=1, cmp_c=0x80808080,\
                            in_cut_start=0, row_ck_on=0, addr_in=0x8380, addr_out=(0x10000+40*128)>>2)
                map_config[0][0][((0, 0), (core_x, core_y))]['soma2'].append(soma2)
            else:
                map_config[0][0][((0, 0), (core_x, core_y))]['soma2'].append(None)

    #***************************************************** phase 4 **************************************************#
    # 接收数据 38 * 128 发送 20 * 256
    if phase_en[3] == 1:
        phase += 1
        for core in range(start_row * M, start_row * M + core_num):
            core_x = core % M
            ...
            if in_data_en == 1:
                soma2 = pX5(core_x=core_x, core_y=core_y, PIC_mode=0, pad_on=False, type_in=1, type_out=1, cin=128, \
                            cout=128, px=19, py=2, kx=1, ky=1, sx=1, sy=1, cmp_c=0x80808080,\
                            in_cut_start=0, row_ck_on=0, addr_in=0x8380, addr_out=(0x10000+60*128)>>2)
                map_config[0][0][((0, 0), (core_x, core_y))]['soma2'].append(soma2)
            else:
                map_config[0][0][((0, 0), (core_x, core_y))]['soma2'].append(None)

    #***************************************************** phase 2 **************************************************#
    # 接收数据 98 * 128 发送 50 * 256
    ...
    if phase_en[7] == 1:
        ...
        for core in range(start_row * M, start_row * M + core_num):
            core_x = core % M
            core_y = core // M
            axon = p41(core_x=core_x, core_y=core_y, px=14, py=7, pad_on=False, load_bias=0, cin=128, cout=32, \
    ...
    if phase_en[9] == 1:
        ...
        for core in range(start_row * M, start_row * M + core_num):
            ...
            axon = p41(core_x=core_x, core_y=core_y, px=14, py=7, pad_on=False, load_bias=0, cin=256, cout=32, \
                       kx=1, ky=1, sx=1, sy=1, addr_ina=0x13100 >> 2, addr_inb=0x0 >> 2, addr_bias=0x0, \
                       addr_out=0x1c400 >> 2)
            # 只计算奇数行
            a = axon.init_data()
            axon.memory_blocks = [
    ...
    # L1多播，X1数据split，只保留32channel作为Xe 0xe780（group9不需要保存）
    if phase_en[12] == 1:
        phase += 1
        for core in range(start_row * M, start_row * M + core_num):
            core_x = core % M
    ...
    # phase_en[0:10] = 1
    shortcut_map_config = Gen_G9_shortcut_Map_Config(phase_en, clock, M=8, N=2, start_row=0, in_data_en=in_data_en, delay_L4=delay_L4, delay_L5=delay_L5)
    for i in range(0, 8):
        for j in range(0, 2):
            final_map_config[0][1][((0, 0), (i, j))] = shortcut_map_config[0][0][((0, 0), (i - 0, j - 0))]
        for j in range(2, 6):
            final_map_config[0][0][((0, 0), (i, j))] = map_config[0][0][((0, 0), (i - 0, j - 2))]

    from generator.test.SCH_Mapping.AddRouterInfo import add_router_info
    final_map_config = add_router_info(map_config=final_map_config, group_idx_list=[0, 1], chip_x_num=1, chip_y_num=1, core_x_num=16, core_y_num=10)

    from generator.test.SCH_Mapping.changeIR_to_prims import change_ir_to_prims
    final_map_config = change_ir_to_prims(map_config=final_map_config, group_idx_list=[0, 1], chip_x_num=1, chip_y_num=1, core_x_num=16, core_y_num=10)

    from generator.test.SCH_Mapping.G9_instant import Gen_G9_instant
    instant_map_config = Gen_G9_instant(start_instant_PI_num = 13, init = False)
    insert_pos = len(final_map_config[0][0][((0, 0), (0, 2))]['prims']) - 1
    for i in range(0, 8):
        for j in range(0, 2):
            final_map_config[0][1][((0, 0), (i, j))]['instant_prims'] = instant_map_config[0][1][((0, 0), (i, j))]['instant_prims']
            final_map_config[0][1][((0, 0), (i, j))]['registers'] = instant_map_config[0][1][((0, 0), (i, j))]['registers']
        for j in range(2, 6):
            final_map_config[0][0][((0, 0), (i, j))]['registers'] = instant_map_config[0][0][((0, 0), (i, j))]['registers']
            final_map_config[0][0][((0, 0), (i, j))]['prims'].insert(insert_pos, instant_map_config[0][0][((0, 0), (i, j))]['prims'][0])



    return final_map_config
...
if run:
    ...
    #
    # phase_en[14] = 0    # L2的结果整理 从14*14*32 -> 14*14*256 发送 - 1 (50 * 256)
    # phase_en[15] = 0    # L2的结果整理 从14*14*32 -> 14*14*256 多播 - 1 (50 * 256)
    # phase_en[16] = 0    # L2的结果整理 从14*14*32 -> 14*14*256 发送 - 2 (48 * 256)
    # phase_en[17] = 0    # L2的结果整理 从14*14*32 -> 14*14*256 多播 - 2 (48 * 256) L3卷积 - 1 (50 * 256) 保存在0x8800
    # phase_en[18] = 0    # L2的结果整理 从14*14*32 -> 14*14*256 发送 - 3 (50 * 256)
    # phase_en[19] = 0    # L2的结果整理 从14*14*32 -> 14*14*256 多播 - 3 (50 * 256) L3卷积 - 2 (48 * 256)
    # phase_en[20] = 0    # L2的结果整理 从14*14*32 -> 14*14*256 发送 - 4 (48 * 256)
    # phase_en[21] = 0    # L2的结果整理 从14*14*32 -> 14*14*256 多播 - 4 (48 * 256) L3卷积 - 3 (50 * 256)
    # phase_en[22] = 0    # L3卷积 - 4 (48 * 256)
    #
    # phase_en[23] = 0    # L3结果与Xe相加，流水relu，保存在Mem0 0x8800
    # phase_en[24] = 0
    # phase_en[25] = 0
    # phase_en[26] = 0
    ...
    map_config = Gen_G9_Map_Config(phase_en, 80000, M=8 , N=4)


    map_config['sim_clock'] = 250_000
    from generator.test.SCH_Mapping.changeIR import changeIR

    map_config = changeIR(map=map_config, chip_x=1, chip_y=1, group_idx_list=[0, 1])
    ...
    # #
    # import pickle
    # with open('sch_test_0810_1007.map_config', 'wb') as f:
