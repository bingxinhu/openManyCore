...
def Gen_G17_Map_Config(phase_en, clock, M, N, start_row=0, in_data_en=0, out_data_en=0, delay_L4=(), delay_L5=()):
    """
    ResNet-50 Group17 Mapping
    ...
    #***************************************************** phase 1 **************************************************#
    # 接收5 * 512的数据，并横向多播，发送1*1*128的数据给Group18
    if phase_en[0] == 1:
        phase += 1
        for core in range(start_row * M, start_row * M + core_num):
            core_x = core % M
            ...
            if core_y == N - 1 and N == 5:#最后一行core是数据发送core，最后一行core也要接收Group15计算完成后发来的数据
                ...
                if out_data_en == 1:
                    if core_y  == 3:
                        ...
                        soma1 = pX6(core_x=core_x, core_y=core_y, type=6, length_in=length, length_ciso=16,
                                    length_out=length, \
                                    num_in=1, num_ciso=1, num_out=1, row_ck_on=0, addr_in=0x1a000 >> 2,
                                    addr_ciso=0x10000 >> 2,
                                    addr_out=0x9000)
                        s1 = soma1.init_data()
                        soma1.memory_blocks = [
                ...
                router = p09(core_x=core_x, core_y=core_y, rhead_mode=1, send_en=0, receive_en=0, addr_dout_base=0x1000, \
                             ...
                             addr_din_base=0x380, addr_din_length=319, receive_num=3, t_mode=1, soma_in_en=0)
                if in_data_en == 1:
                    router.Receive_en = 1
                ...
                if out_data_en == 1:
                    if core_y == 3:
                        router.Send_en = 1
                        router.Send_number = 1*1*128 // 8 - 1
                        router.Soma_in_en = 1
                        dst_x = -1  # dst is ((1, 0), (15, 0))
                        dst_y = -1
                        A = core_x * 16
                ...
                if in_data_en == 0 and (out_data_en == 0 or core_y != 3):
                    router = None
    ...
    if phase_en[3] == 1:
        ...
        for core in range(start_row * M, start_row * M + core_num):
            ...
            if core_y == N - 1 and N == 5:#最后一行core是数据发送core，最后一行core也要接收Group15计算完成后发来的数据
                ...
                router = p09(core_x=core_x, core_y=core_y, rhead_mode=1, send_en=0, receive_en=0, addr_dout_base=0x1000, \
                             addr_dout_length=0, send_num=0, addr_rhead_base=rhead_base, addr_rhead_length=0, cxy=0, \
                             addr_din_base=0x380, addr_din_length=639, receive_num=3, t_mode=1, soma_in_en=0)
                if in_data_en == 1:
                    router.Receive_en = 1
                    if core_y - start_row == 0:
                        if core_x == 0:
        ...
        for core in range(start_row * M, start_row * M + core_num):
            ...
            if core_y == N - 1 and N == 5:  # 最后一行core是数据发送core，最后一行core也要接收Group15计算完成后发来的数据
                ...
                router = p09(core_x=core_x, core_y=core_y, rhead_mode=1, send_en=0, receive_en=1, addr_dout_base=0x1000, \
                             addr_dout_length=0, send_num=0, addr_rhead_base=rhead_base, addr_rhead_length=0, cxy=0, \
                             addr_din_base=0x380, addr_din_length=1535, receive_num=11, t_mode=1, soma_in_en=1)
                if in_data_en == 1:
                    router.Receive_en = 1
                    if core_y - start_row == 0:
                        if core_x == 0:
    ...
    if phase_en[8] == 1:
        ...
        for core in range(start_row * M, start_row * M + core_num):
            ...
            if core_y == N - 1 and N == 5:  # 最后一行core是不进行计算
                ...
                soma2 = pX6(core_x=core_x, core_y=core_y, type=26, length_in=128, length_ciso=16, length_out=128, \
        ...
        for core in range(start_row * M, start_row * M + core_num):
            ...
            if core_y == N - 1 and N == 5:  # 最后一行core是不进行计算
                ...
                if core_y - start_row == 0:
                    ...
                    addr_out = 0x8000 >> 2
                soma2 = pX6(core_x=core_x, core_y=core_y, type=26, length_in=3584, length_ciso=16, length_out=3584, \
    ...
    # ***************************************************** phase 13 **************************************************#
    """
        46层与shorcut结果求和，流水relu，流水发送到第四行的core
    """
    if phase_en[16] == 1:
        phase += 1
        for core in range(start_row * M, start_row * M + core_num):
            ...
            if core_y == N - 1 and N == 5:  # 最后一行core是不进行计算
                ...
                soma1 = pX5(core_x=core_x, core_y=core_y, PIC_mode=0, pad_on=False, type_in=0, type_out=1, cin=128, \
                            ...
                            in_cut_start=2, row_ck_on=1, addr_in=axon.Addr_V_base, addr_out=0x9000, in_row_max=7)
                map_config[0][0][((0, 0), (core_x, core_y))]['soma1'].append(soma1)
                    ...
                    if map_config[0][0][((0, 0), (core_x, core_y))]['router'][temp_phase - 1] != None:
                        ...
                        if map_config[0][0][((0, 0), (core_x, core_y))]['router'][temp_phase - 1].Send_en == 1:
                            rhead_base += (map_config[0][0][((0, 0), (core_x, core_y))]['router'][
                                               temp_phase - 1].Addr_Rhead_length + 1) * 4
                router = p09(core_x=core_x, core_y=core_y, rhead_mode=1, send_en=1, receive_en=0, cxy=0, \
                             addr_dout_base=0x1000, addr_dout_length=0, send_num=223 if core_y != 3 else 111\
                             , addr_rhead_base=rhead_base, \
                             addr_rhead_length=0, addr_din_base=0x400, addr_din_length=0, receive_num=3, \
                             t_mode=1, soma_in_en=1)
                if core_y - start_row == 0:
                    ...
                    router.Receive_en = 1
                    router.Addr_Din_length = int(7 * 7 * 128 // 8 - 1)
                    router.addRHead(S=0, T=1, P=0, Q=0, X=0, Y=-0, A=672, pack_per_Rhead=111, A_offset=0, Const=0, EN=1)
                map_config[0][0][((0, 0), (core_x, core_y))]['router'].append(router)

                soma2 = pX6(core_x=core_x, core_y=core_y, type=26, length_in=3136, length_ciso=16, length_out=3136, \
                            num_in=2, num_ciso=2, num_out=2, row_ck_on=0, addr_in=0x8400, addr_ciso=0x1fc00 >> 2,
                            addr_out=0x8000 >> 2)
                if core_y - start_row == 3:
                    map_config[0][0][((0, 0), (core_x, core_y))]['soma2'].append(soma2)
                else:
    ...
    # ***************************************************** phase 14 **************************************************#
    """
        第四行的core进行7*7平均池化，结果保存在Mem1 0x1a000 >> 2
    """
    if phase_en[17] == 1:
        phase += 1
        for core in range(start_row * M, start_row * M + core_num):
            ...
            if core_y == N - 1 and N == 5:  # 最后一行core是不进行计算
                ...
                axon = p02(core_x=core_x, core_y=core_y, avf_pooling_en=True, pad_on=False, ina_type=1, load_bias=0, \
                           cin=128, px=7, py=7, kx=7, ky=7, sx=1, sy=1, addr_in=0x8000 >> 2, \
                           addr_bias=0x0, addr_out=0x19000 >> 2)
                if phase_en[12] == 0:
                    a = axon.init_data()
                    axon.memory_blocks = [
                        {'name': "core_{:d}_{:d}_phase_14_InA".format(core_x, core_y),
                         'start': axon.Addr_InA_base,
                         'length': axon.cin * axon.Input_fm_Px * axon.Input_fm_Py // 4,
                         'data': a[0],
                         'mode': 0}
                    ]
                if core_y - start_row == 3:
                ...
                soma1 = pX5(core_x=core_x, core_y=core_y, PIC_mode=0, pad_on=False, type_in=0, type_out=1, cin=128, \
                            cout=128, px=1, py=1, kx=1, ky=1, sx=1, sy=1, cmp_c=0x80808080, \
                            in_cut_start=0, row_ck_on=1, addr_in=axon.Addr_V_base, addr_out=0x1a000 >> 2, in_row_max=1)
                if core_y - start_row == 3:
                    map_config[0][0][((0, 0), (core_x, core_y))]['soma1'].append(soma1)
                else:
                    map_config[0][0][((0, 0), (core_x, core_y))]['soma1'].append(None)
...
phase_en[2] = 0     # 44卷积计算，部分和收发
phase_en[3] = 0     # 44部分和求和
phase_en[24] = 0    # 44多播
phase_en[4] = 1     # X1数据整理成Xe

phase_en[5] = 0     # 45层卷积计算   --- 时钟较大 2.2k可以满足
phase_en[6] = 0     # 45部分和求和
phase_en[25] = 0    # 45层多播
...
phase_en[12] = 1    # 46层输出与Xe相加，流水relu，发送到第四行的core中
phase_en[13] = 1    # 第四行的core进行平均池化，结果保存在Mem0 0x8000

run = True  # 只生成map_config 还是生成后直接运行

map_config = Gen_G17_Map_Config(phase_en, 22000, M=1, N=5)
