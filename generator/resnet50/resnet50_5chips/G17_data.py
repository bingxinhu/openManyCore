...
def generate_g17_data(handler, size_y, size_x):
    data = {
        'layer4.2.conv1': {},
        'layer4.2.relu1': {},
        'layer4.2.cut1': {},
    ...
    l1_weight_split_dict, l1_bias_split_dict, l1_output_split_dict = {}, {}, {}
    l1_input_split_dict = {}
    l1_input_split_dict1, l1_input_split_dict3 = {}, {}
    l1_output_split_dict2, l1_output_split_dict3 = {}, {}
    l1_input_split_dict2 = {}
    l1_input_raw_data = handler.parameters['layer4.2.conv1']['input'].astype(np.int8).reshape(2048, -1, 1)  # 2048, 7, 7
    l1_weight_raw_data = handler.parameters['layer4.2.conv1']['weight'].astype(np.int8)
    l1_bias_raw_data = np.append(
        handler.parameters['layer4.2.conv1']['bias'].astype(np.int32),
        np.zeros_like(handler.parameters['layer4.2.conv1']['bias']).astype(np.int32))
    l1_output_raw_data = handler.parameters['layer4.2.cut1']['output'].astype(np.int8).reshape(512, -1, 1)  # 512, 49, 1
    ...
    for core_y, core_x in product(range(size_y), range(size_x)):
        l1_input_split_dict[(core_x, core_y)] = ((core_y * 512, (core_y + 1) * 512), (0, l1_ih), (0, l1_iw))
    ...
    data['layer4.2.conv1']['weight'] = ResNetDataHandler.tensor_split(
    ...
    l2_weight_raw_data = handler.parameters['layer4.2.conv2']['weight'].astype(np.int8)
    l2_bias_raw_data = np.append(handler.parameters['layer4.2.conv2']['bias'].astype(np.int32),
                                 np.zeros_like(handler.parameters['layer4.2.conv2']['bias'], dtype=np.int32))
    l2_output_raw_data = handler.parameters['layer4.2.cut2']['output'].astype(np.int8)
    ...
    l3_input_raw_data = handler.parameters['layer4.2.conv3']['input'].astype(np.int8)
    l3_weight_raw_data = handler.parameters['layer4.2.conv3']['weight'].astype(np.int8)
    l3_bias_raw_data = np.append(handler.parameters['layer4.2.conv3']['bias'].astype(np.int32),
                                 np.zeros_like(handler.parameters['layer4.2.conv3']['bias'], dtype=np.int32))
    l3_output_raw_data = handler.parameters['layer4.2.cut3']['output'].astype(np.int8).reshape((2048, -1, 1))
    ...
    y_output_split_dict, y_output_split_dict2 = {}, {}
    avg_pool_out_split_dict = {}
    y_output_raw_data = handler.parameters['layer4.2.cut5']['output'].astype(np.int8).reshape((2048, -1, 1))
    avg_pool_raw_data = handler.parameters['avgpool_cut']['output'].astype(np.int8)
    (_, y_oh, y_ow) = y_output_raw_data.shape
    (_, avg_oh, avg_ow) = avg_pool_raw_data.shape
    for core_y, core_x in product(range(size_y), range(size_x)):
        if core_y == 3:
            ...
            y_output_split_dict[(core_x, core_y)] = ((core_x * 128, core_x * 128 + 128),
                                                     ...
                                                     (0, y_ow))
        if core_y == 0:
            y_output_split_dict2[(core_x, core_y)] = ((core_x * 128, core_x * 128 + 128),
                                                      (0, y_oh),
                                                      (0, y_ow))
        else:
            y_output_split_dict2[(core_x, core_y)] = ((0, 0), (0, 0), (0, 0))
        avg_pool_out_split_dict[(core_x, core_y)] = ((core_x * 128, core_x * 128 + 128), (0, avg_oh), (0, avg_ow))
    data['layer4.2.cut5']['output'] = ResNetDataHandler.tensor_split(
        raw_data=y_output_raw_data, split_dict=y_output_split_dict, data_type=1, alignment=(16, None, None),
        dims=(0, 2, 1))
    data['layer4.2.cut5']['output2'] = ResNetDataHandler.tensor_split(
        raw_data=y_output_raw_data, split_dict=y_output_split_dict2, data_type=1, alignment=(16, None, None),
        dims=(0, 2, 1))
    data['avgpool']['output'] = ResNetDataHandler.tensor_split(
        raw_data=avg_pool_raw_data, split_dict=avg_pool_out_split_dict, data_type=1, alignment=(16, None, None),
        dims=(0, 2, 1))
