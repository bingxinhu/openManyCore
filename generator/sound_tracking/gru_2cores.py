...
from generator.mapping_utils.prims import pX5, p02, p03, p83, p07, p04, p09, p06, p26
...
def gen_gru_map_config(phase_en, clock_in_phase, size_x, size_y, in_cut_start_mat=None, data=None, handler=None,
    ...
    #             axon = None
    #             soma1 = p06(addr_in=0x0000 >> 2, addr_out=0x24000 >> 2, addr_ciso=0x1ff00 >> 2, length_in=39 * 16,
    ...
    #             soma2 = p26(addr_in=0x21000 >> 2, addr_out=0x0000 >> 2, addr_ciso=0xff00 >> 2,
    ...
    if phase_en[offset + 0]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            if core_id < handler.sequence_length:
                ...
                addr_in = 0x0000 >> 2 if core_x == 0 else 0x0260 >> 2   # 正向和反向
                axon = p04(cin=16, cout=128, addr_ina=addr_in, addr_inb=0x10000 >> 2,
                           addr_bias=0x10800 >> 2,
                ...
                phase_group[(chip, (core_x, core_y))]['prims'].append({'axon': axon, 'soma1': soma1, 'router': router,
                                                                       'soma2': soma2})
                # core register
                phase_group[(chip, (core_x, core_y))]['registers'] = {
    ...
    if phase_en[offset + 1]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            if core_id < handler.sequence_length:
                ...
                axon = p04(cin=128, cout=128, addr_ina=0xff00 >> 2, addr_inb=0x10a00 >> 2, addr_bias=0x14a00 >> 2,
    ...
    if phase_en[offset + 4]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            if core_id < handler.sequence_length:
                ...
                axon = p04(cin=128, cout=128, addr_ina=0xff00 >> 2, addr_inb=0x15700 >> 2, addr_bias=0x19700 >> 2,
                           addr_out=0x0480 >> 2, ina_type=1, inb_type=1, load_bias=2,
                           data_x=None, data_w=data_w, data_b=data_b)
                soma1 = None
                router = None
    ...
    if phase_en[offset + 7]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            if core_id < handler.sequence_length:
                ...
                axon = p04(cin=128, cout=128, addr_ina=0xff00 >> 2, addr_inb=0x1a300 >> 2, addr_bias=0x1e300 >> 2,
                           addr_out=0x0480 >> 2, ina_type=1, inb_type=1, load_bias=2,
                           data_x=None, data_w=data_w, data_b=data_b)
                soma1 = None
                router = None
    ...
    if phase_en[offset + 12]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            if core_id < handler.sequence_length:
                axon = p03(tensor_en=False, x_array_num=1, cin=128, tensor_px=1, tensor_py=1, tensor_sx=1, tensor_sy=1,
                           addr_ina=0x0900 >> 2, addr_inb=0xff00 >> 2, addr_bias=0x0000 >> 2,
                           addr_out=0x0480 >> 2, ina_type=1, load_bias=0, bias_length=0,
                           data_x=None, data_y=None, data_b=None)
                soma1 = None
                router = None
                # Move data_in for next loop
                soma2 = p06(addr_in=0x0000 >> 2, addr_out=0x2000 >> 2, addr_ciso=0x10000 >> 2, length_in=39 * 16,
                            num_in=1, length_ciso=1, num_ciso=1, length_out=39 * 16, num_out=1, type_in=1, type_out=1,
    ...
    if phase_en[offset + 13]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            if core_id < handler.sequence_length:
                ...
                # Move data_in for next loop
                if core_x == 0:
                    addr_in, addr_out = (0x2000 + 16) >> 2, 0x2000 >> 2
                else:
                    addr_in, addr_out = 0x0000 >> 2, (0x0000 + 16) >> 2
                soma2 = p06(addr_in=addr_in, addr_out=addr_out, addr_ciso=0x10000 >> 2, length_in=39 * 16,
                            num_in=1, length_ciso=1, num_ciso=1, length_out=39 * 16, num_out=1, type_in=1, type_out=1,
                            data_in=None, data_ciso=None)
                phase_group[(chip, (core_x, core_y))]['prims'].append({'axon': axon, 'soma1': soma1, 'router': router,
                                                                       'soma2': soma2})
...
if __name__ == '__main__':
    ...
    clock_in_phase = 2_000
    config = gen_gru_map_config(phase, clock_in_phase=clock_in_phase, size_x=2, size_y=1, data=gru_data,
