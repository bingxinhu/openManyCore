...
def gen_g4_map_config(phase_en, clock_in_phase, size_x, size_y, cuts, data=None,
    ...
    if phase_en[0]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            if (core_x, core_y) != (0, 0):
                axon = None
            soma1 = None
            if out_data_en and (core_x, core_y) == (0, 0):
                soma1 = p06(addr_in=0x9000 >> 2, addr_out=0x9000, addr_ciso=0x0, length_in=256,
                            length_out=256, length_ciso=256, num_in=1, num_ciso=1, num_out=1, type_in=1,
                            type_out=1, in_cut_start=0, in_row_max=0, row_ck_on=0,
                            data_in=data['avgpool_cut']['output'][(core_x, core_y)] if init_data else None,
                            data_ciso=None)
            addr_rhead_base = MapConfigGen.get_router_rhead_base(phase_group[(chip, (core_x, core_y))]['prims'],
                                                                 limit=0x380)
            router = p09(rhead_mode=1, send_en=out_data_en and (core_x, core_y) == (0, 0),
                         receive_en=in_data_en, send_num=31, receive_num=0,
            ...
            if out_data_en and (core_x, core_y) == (0, 0):
                dst_x, dst_y = -2, 3
                router.addRHead(S=0, T=1, P=0, Q=1, X=dst_x - core_x, Y=dst_y - core_y,
                                A=0, pack_per_Rhead=31, A_offset=0, Const=0, EN=1)
    ...
    # max pool // 1 * 28 -> 1 * 7 // 4个core发送到一起，继续max pool
    if phase_en[offset + 9]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            axon = None
            soma1 = pX5(mode='max', addr_in=0x9000 >> 2, addr_out=0x9000, cin=256, cout=256, px=28, py=1,
                        kx=4, ky=1, sx=4, sy=1, cmp_c=0x80808080, type_in=1, type_out=1,
                        in_cut_start=0, row_ck_on=0, in_row_max=1)
            if core_x % 2 == 0 and core_y == 0:
                receive_en = 1
            else:
                receive_en = 0
            dst_x, dst_y = (core_x // 2) * 2, 0
            addr_rhead_base = MapConfigGen.get_router_rhead_base(phase_group[(chip, (core_x, core_y))]['prims'])
            router = p09(rhead_mode=1, send_en=1, receive_en=receive_en, send_num=223, receive_num=3,
                         addr_din_base=0x380,
                         addr_din_length=895, addr_rhead_base=addr_rhead_base, addr_rhead_length=0,
                         addr_dout_base=0x1000, addr_dout_length=0, soma_in_en=1, data_in=None)
            router.addRHead(S=0, T=1, P=0, Q=0, X=dst_x - core_x, Y=dst_y - core_y,
                            A=224 * ((core_x % 2) + core_y * 2), pack_per_Rhead=223, A_offset=0, Const=0, EN=1)
            soma2 = p06(addr_in=0x8380, addr_out=0x18b80 >> 2, addr_ciso=0x0, length_in=256, length_out=256,
                        length_ciso=1, num_in=28, num_ciso=28, num_out=28, type_in=1, type_out=1,
                        in_cut_start=0, in_row_max=0, row_ck_on=0, data_in=None, data_ciso=None)
            phase_group[(chip, (core_x, core_y))]['prims'].append({'axon': axon, 'soma1': soma1, 'router': router,
                                                                   'soma2': soma2})

    # max pool // 4 * 7 -> 1 * 7 // 7个core发送到一起，继续avg pool
    if phase_en[offset + 10]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            axon = None
            soma1 = pX5(mode='max', addr_in=0x18b80 >> 2, addr_out=0x9000, cin=256, cout=256, px=7, py=4,
                        kx=1, ky=4, sx=1, sy=1, cmp_c=0x80808080, type_in=1, type_out=1,
                        in_cut_start=0, row_ck_on=0, in_row_max=1)
            receive_en = 1 if (core_x, core_y) == (0, 0) else 0
            send_en = 1 if core_x % 2 == 0 and core_y == 0 else 0
            addr_rhead_base = MapConfigGen.get_router_rhead_base(phase_group[(chip, (core_x, core_y))]['prims'])
            router = p09(rhead_mode=1, send_en=send_en, receive_en=receive_en, send_num=223, receive_num=6,
                         addr_din_base=0x380,
                         addr_din_length=1567, addr_rhead_base=addr_rhead_base, addr_rhead_length=0,
                         addr_dout_base=0x1000, addr_dout_length=0, soma_in_en=1, data_in=None)
            router.addRHead(S=0, T=1, P=0, Q=0, X=0 - core_x, Y=0 - core_y,
                            A=224 * (core_x // 2), pack_per_Rhead=223, A_offset=0, Const=0, EN=1)
            soma2 = p06(addr_in=0x8380, addr_out=0x18b80 >> 2, addr_ciso=0x0, length_in=256, length_out=256,
                        length_ciso=1, num_in=49, num_ciso=49, num_out=49, type_in=1, type_out=1,
                        in_cut_start=0, in_row_max=0, row_ck_on=0, data_in=None, data_ciso=None)
            if not (core_x % 2 == 0 and core_y == 0):
                axon, soma1, router, soma2 = None, None, None, None
            phase_group[(chip, (core_x, core_y))]['prims'].append({'axon': axon, 'soma1': soma1, 'router': router,
                                                                   'soma2': soma2})
    ...
    if phase_en[offset + 11]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            axon = p02(avg_pooling_en=True, ina_type=1, load_bias=0, kx=7, ky=7, sx=1, sy=1, cin=256, px=7, py=7,
                       addr_in=0x18b80 >> 2, addr_bias=0x0, addr_out=0x2000 >> 2, pad_top=0, pad_down=0, pad_left=0,
                       pad_right=0, bias_length=0, data_x=None, data_b=None, constant_b=0)
            soma1 = pX5(mode='max', addr_in=0x2000 >> 2, addr_out=0x9000 >> 2, cin=256, cout=256, px=1, py=1,
                        kx=1, ky=1, sx=1, sy=1, cmp_c=0x80808080, type_in=0, type_out=1,
                        in_cut_start=cuts['avgpool_cut'], row_ck_on=1, in_row_max=1)
