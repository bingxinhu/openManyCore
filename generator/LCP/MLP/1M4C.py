...
def gen_1M4C_map_config(phase_en, clock_in_phase, size_x, size_y, delay_l4=None, delay_l5=None,
    ...
    if phase_en[offset + 0]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            axon = p04(cin=16, cout=512, addr_ina=0x0000 >> 2, addr_inb=0x10000 >> 2, addr_bias=0x12000 >> 2,
    ...
    if phase_en[offset + 1]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            axon = p04(cin=512, cout=32, addr_ina=0x0020 >> 2, addr_inb=0x12800 >> 2, addr_bias=0x16800 >> 2,
                       addr_out=0x0220 >> 2, ina_type=1, inb_type=1, load_bias=2,
            ...
            axon = p41(px=1, py=1, cin=axon.cin, cout=axon.cout, kx=1, ky=1, sx=1, sy=1,
                       ...
                       data_x=None, data_w=[], data_b=[])
            soma1 = p06(addr_in=0x0220 >> 2, addr_out=0x24000 >> 2,
            ...
            if core_x == 0 and core_y == 0:
                soma1 = None
                soma2 = p06(addr_in=0x21000 >> 2, addr_out=0x02A0 >> 2,
                            addr_ciso=0x0000 >> 2, length_in=32,
                            length_out=32, length_ciso=1, num_in=3, num_ciso=3, num_out=3, type_in=0,
        ...
        for core_y, core_x in product(range(size_y), range(size_x)):
            if core_x == 0 and core_y == 0:
                axon = p02(avg_pooling_en=False, ina_type=0, load_bias=0, kx=1, ky=4, sx=1, sy=1, cin=32, px=1, py=1,
                           addr_in=0x0220 >> 2, addr_bias=0x0, addr_out=0x8000 >> 2, pad_top=0, pad_down=0, pad_left=0,
                           pad_right=0, bias_length=0, data_x=None, data_b=None, constant_b=0)
                soma1 = pX5(mode='max', addr_in=0x8000 >> 2, addr_out=0x0420 >> 2,
...
if __name__ == '__main__':
    ...
    config = gen_1M4C_map_config(phase, clock_in_phase=clock_in_phase, size_x=4, size_y=1,
