...
def gen_g16_map_config(phase_en, clock_in_phase, size_x, size_y, cuts, data=None,
                       in_data_en=False, out_data_en=False, delay_l4=None, delay_l5=None, chip=(0, 0), init_data=None):
    """
        ResNet-50 5-Chip Group-16
        core array : 4 * 16
    """
    map_config = {
        'sim_clock': None,
    ...
    # 接收 24*512/1024=12.5    **  发送 12, 12, 12, 13 * 128
    if phase_en[0]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            if out_data_en and (core_y in [0, 1]):
                soma1 = pX5(mode='max', addr_in=0x1f200 >> 2, addr_out=0x9000, cin=128, cout=128, px=12, py=1, kx=1,
                            ky=1, sx=1, sy=1, cmp_c=0x80808080, type_in=1, type_out=1, in_cut_start=0, row_ck_on=0,
                            in_row_max=1,
                            data_in=data['layer4.1.cut5']['output'][(core_x, core_y)] if init_data else None)
            ...
            router = p09(rhead_mode=1, send_en=out_data_en and (core_y in [0, 1]),
                         receive_en=in_data_en, send_num=191, receive_num=0,
            ...
            if out_data_en and (core_y in [0, 1]):
                ...
                router.addRHead(S=0, T=1, P=0, Q=1, X=dst_x - core_x, Y=dst_y - core_y, A=a, pack_per_Rhead=191,
    ...
    if phase_en[1]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            if out_data_en and (core_y in [2, 3]):
                ...
                soma1 = pX5(mode='max', addr_in=0x1f200 >> 2, addr_out=0x9000, cin=128, cout=128, px=px, py=1, kx=1,
                            ky=1, sx=1, sy=1, cmp_c=0x80808080, type_in=1, type_out=1, in_cut_start=0, row_ck_on=0,
                            in_row_max=1,
                            data_in=data['layer4.1.cut5']['output'][(core_x, core_y)] if init_data else None)
            addr_rhead_base = MapConfigGen.get_router_rhead_base(phase_group[(chip, (core_x, core_y))]['prims'],
            ...
            router = p09(rhead_mode=1, send_en=out_data_en and (core_y in [2, 3]),
                         receive_en=in_data_en, send_num=px * 128 // 8 - 1, receive_num=0,
    ...
    if phase_en[offset + 0]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            axon = p41(px=7, py=7, cin=512, cout=32, kx=1, ky=1, sx=1, sy=1, addr_ina=0x19000 >> 2,
                       ...
                       data_x=None if in_data_en else data['layer4.1.conv1']['input'][(core_x, core_y)],
                       data_w=data['layer4.1.conv1']['weight'][(core_x, core_y)],
                       data_b=data['layer4.1.conv1']['bias'][(core_x, core_y)])
    ...
    if phase_en[offset + 1]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            if core_y in [0]:
                px, py = 13, 1      # 此处行流水也出错了 不仅中间缓冲区错，结果也是错的
            else:
                px, py = 12, 1
            ...
            soma2 = pX5(mode='max', addr_in=0x1f200 >> 2, addr_out=0x8300 >> 2, cin=32, cout=32, px=px, py=py, kx=1,
                        ky=1, sx=1, sy=1, cmp_c=0x0, type_in=0, type_out=1, in_cut_start=cuts['layer4.1.cut1'],
    ...
    if phase_en[offset + 5]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            axon = p41(px=7, py=7, cin=128, cout=32, kx=3, ky=3, sx=1, sy=1, addr_ina=0x8300 >> 2,
                       ...
                       data_x=None,
                       data_w=data['layer4.1.conv2']['weight'][(core_x, core_y)],
                       data_b=data['layer4.1.conv2']['bias'][(core_x, core_y)])
    ...
    if phase_en[offset + 6]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            soma2 = pX5(mode='max', addr_in=0x8300 >> 2, addr_out=0x19000 >> 2, cin=32, cout=32, px=px, py=py, kx=1,
                        ky=1, sx=1, sy=1, cmp_c=0x0, type_in=0, type_out=1, in_cut_start=cuts['layer4.1.cut2'],
                        row_ck_on=0, in_row_max=1)
            phase_group[(chip, (core_x, core_y))]['prims'].append({'axon': axon, 'soma1': soma1, 'router': router,
                                                                   'soma2': soma2})
    ...
    if phase_en[offset + 9]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            axon = p41(px=4, py=6, cin=128, cout=128, kx=1, ky=1, sx=1, sy=1, addr_ina=0x19000 >> 2,
                       ...
                       load_bias=2, pad_top=0, pad_down=0, pad_left=0, pad_right=0,
                       data_x=None,
                       data_w=data['layer4.1.conv3']['weight'][(core_x, core_y)],
                       data_b=data['layer4.1.conv3']['bias'][(core_x, core_y)])
    ...
    if phase_en[offset + 10]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            soma1 = pX5(mode='max', addr_in=0x1a880 >> 2, addr_out=0x8300 >> 2, cin=128, cout=128, px=6, py=1, kx=1,
                        ky=1, sx=1, sy=1, cmp_c=0x80808080, type_in=0, type_out=1, in_cut_start=cuts['layer4.1.cut3'],
    ...
    # Y46 进行整理，每个得到13*128， 12*128， 12*128， 13*128
    ...
    if phase_en[offset + 16]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            axon = p02(avg_pooling_en=False, ina_type=1, load_bias=0, kx=1, ky=2, sx=1, sy=1, cin=128, px=px, py=1,
                       ...
                       pad_right=0, bias_length=0, data_x=None, data_b=None, constant_b=0)
            soma1 = pX5(mode='max', addr_in=0x19000 >> 2, addr_out=0x1f200 >> 2, cin=128, cout=128, px=px, py=1, kx=1,
                        ky=1, sx=1, sy=1, cmp_c=0x0, type_in=0, type_out=1, in_cut_start=cuts['layer4.1.cut5'],
                        row_ck_on=1, in_row_max=1)
            router = None
            soma2 = None
            phase_group[(chip, (core_x, core_y))]['prims'].append({'axon': axon, 'soma1': soma1, 'router': router,
...
if __name__ == '__main__':
    ...
    phase[offset + 14] = 1  # L46 + shortcut 部分和求和，流水ReLU
    ...
    config = gen_g16_map_config(phase, clock_in_phase=clock_in_phase, size_x=16, size_y=4, data=data, in_data_en=False,
