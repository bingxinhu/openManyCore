...
def check(ref_data, compare: ResultCompare, size_y, size_x, chip=(0, 0), empty_num=0):
    ...
    for core_y, core_x in product(range(*size_y), range(*size_x)):
        num_in = 11264 if core_x == 3 else 10240
        data[empty_num + 0][chip, (core_x, core_y)] = [{
            'data': ref_data['fpga']['input1'][(core_x - size_x[0], core_y - size_y[0])],
            'addr_start': 0x0000 >> 2,
            'type': 1
        }]
        data[empty_num + 1][chip, (core_x, core_y)] = [{
            'data': ref_data['fpga']['input2'][(core_x - size_x[0], core_y - size_y[0])],
            'addr_start': (0x0000 + num_in * 1) >> 2,
            'type': 1
        }]
        data[empty_num + 2][chip, (core_x, core_y)] = [{
            'data': ref_data['fpga']['input3'][(core_x - size_x[0], core_y - size_y[0])],
            'addr_start': (0x0000 + num_in * 2) >> 2,
            'type': 1
        }]
        data[empty_num + 3][chip, (core_x, core_y)] = [{
            'data': ref_data['fpga']['input4'][(core_x - size_x[0], core_y - size_y[0])],
            'addr_start': (0x0000 + num_in * 3) >> 2,
            'type': 1
        }]
    ...
    for core_y, core_x in product(range(*size_y), range(*size_x)):
        ...
        #     'data': ref_data['conv1']['input'][(core_x - size_x[0], core_y - size_y[0])],
