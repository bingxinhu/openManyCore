...
class Prim_03_Axon(Primitive):
    ...
    def init_data(self) -> list:
        ...
        self.InputX1_array = np.zeros(InputX_size_equal).astype(np.int64)
        self.InputX2_array = np.zeros(InputX_size_equal).astype(np.int64)

        if self.InA_type == 2:
            InputX1_array = data_generator.random_array_u8(InputX_size).astype(np.int64)
            InputX2_array = data_generator.random_array_u8(InputX_size).astype(np.int64)

        else:
            InputX1_array = data_generator.random_array_8(InputX_size).astype(np.int64)
            InputX2_array = data_generator.random_array_8(InputX_size).astype(np.int64)

        if self.tensor_en:
            self.InputX1_array[:, :, :self.cin] = InputX1_array[:, :, :]
            self.InputX2_array[:, :, :self.cin] = InputX2_array[:, :, :]
        else:
            self.InputX1_array[:, :self.cin] = InputX1_array[:, :]
            self.InputX2_array[:, :self.cin] = InputX2_array[:, :]
        ...
        if self.tensor_en:
                ...
                for px in range(self.Px):
                    for i in range(self.cin_wr_real // num_in_4B):
                        ...
                        for j in range(num_in_4B):
                            _tmp.append(int(self.InputX1_array[py][px][num_in_4B * i + j]))
                            pass
                        _data.append(_tmp)
                        pass
            ...
            for py in range(self.Py):
                for px in range(self.Px):
                    for i in range(self.cin_wr_real // num_in_4B):
                        _tmp = []
                        for j in range(num_in_4B):
                            _tmp.append(int(self.InputX2_array[py][px][num_in_4B * i + j]))
                            pass
                        _data.append(_tmp)
                        pass
            _retLsit.append(_data)
            ...
            for px in range(self.X_array_num):
                for i in range(self.cin_wr_real // num_in_4B):
                    _tmp = []
                    for j in range(num_in_4B):
                        _tmp.append(int(self.InputX1_array[px][num_in_4B * i + j]))
                        pass
                    _data.append(_tmp)
                    pass
            _retLsit.append(_data)
            ...
            for px in range(self.X_array_num):
                for i in range(self.cin_wr_real // num_in_4B):
                    _tmp = []
                    for j in range(num_in_4B):
                        _tmp.append(int(self.InputX2_array[px][num_in_4B * i + j]))
                        pass
                    _data.append(_tmp)
                    pass
    ...
    def getInfoList(self) -> list:
        ...
        if self.tensor_en:
            self.Read_X1_length = self.Py * self.Px * (math.ceil(self.cin_wr_real / 32) * 8)
            self.Read_X2_length = self.Py * self.Px * (math.ceil(self.cin_wr_real / 32) * 8)
        else:
            self.Read_X1_length = self.X_array_num * (math.ceil(self.cin_wr_real / 32) * 8)
            self.Read_X2_length = self.X_array_num * (math.ceil(self.cin_wr_real / 32) * 8)

        self.Read_Bias_length = math.ceil(self.Bias_length / 32) * 32
        _infoList = []
        _X1 = {
            "start": self.Addr_InA_base,
            "length": self.Read_X1_length,
            "type": self.InA_type
        }
        _infoList.append(_X1)
        _X2 = {
            "start": self.Addr_InB_base,
            "length": self.Read_X2_length,
            "type": self.InA_type
        }
        _infoList.append(_X2)
        if self.Load_Bias == 2 or self.Load_Bias == 3:
            _B = {
                "start": self.Addr_Bias_base,
        ...
        if self.tensor_en:
            for py in range(self.Py):
                for px in range(self.Px):
                    for i in range(self.cin_wr_real // num_in_4B):
                        for j in range(num_in_4B):
                            self.InputX1_array[py][px][num_in_4B * i + j] = \
            ...
            for py in range(self.Py):
                for px in range(self.Px):
                    for i in range(self.cin_wr_real // num_in_4B):
                        for j in range(num_in_4B):
                            self.InputX2_array[py][px][num_in_4B * i + j] = \
                                inputList[1][int(py * perPy_in_4B + px * perPx_in_4B + i)][j]
                        pass
        else:
            for px in range(self.X_array_num):
                for i in range(self.cin_wr_real // num_in_4B):
                    for j in range(num_in_4B):
                        self.InputX1_array[px][num_in_4B * i + j] = \
            ...
            for px in range(self.X_array_num):
                for i in range(self.cin_wr_real // num_in_4B):
                    for j in range(num_in_4B):
                        self.InputX2_array[px][num_in_4B * i + j] = \
                            inputList[1][int(px * perPx_in_4B + i)][j]
                    pass
        ...
        if self.tensor_en:
            ...
            if self.Load_Bias == 0 or self.Load_Bias == 1:
                for oy_cnt in range(self.Oy):
                    for ox_cnt in range(self.Ox):
                        for cin_cnt in range(self.cin_wr_real):
                            self.result[oy_cnt][ox_cnt][cin_cnt] = self.constant_b + \
                                                                   self.InputX1_array[oy_cnt * self.stride_y][
                                                                       ox_cnt * self.stride_x][cin_cnt] * \
                                                                   self.InputX2_array[oy_cnt * self.stride_y][
                                                                       ox_cnt * self.stride_x][cin_cnt]
                ...
                for oy_cnt in range(self.Oy):
                    for ox_cnt in range(self.Ox):
                        for cin_cnt in range(self.cin_wr_real):
                            self.result[oy_cnt][ox_cnt][cin_cnt] = self.Bias_array[cin_cnt] + \
                                                                   self.InputX1_array[oy_cnt * self.stride_y][
                                                                       ox_cnt * self.stride_x][cin_cnt] * \
                                                                   self.InputX2_array[oy_cnt * self.stride_y][
                                                                       ox_cnt * self.stride_x][cin_cnt]
            ...
            if self.Load_Bias == 0 or self.Load_Bias == 1:
                for i in range(self.X_array_num):
                    for cin_cnt in range(self.cin_wr_real):
                        self.result[i][cin_cnt] = self.constant_b + self.InputX1_array[i][cin_cnt] * \
                                                  self.InputX2_array[i][cin_cnt]
                ...
                for i in range(self.X_array_num):
                    for cin_cnt in range(self.cin_wr_real):
                        self.result[i][cin_cnt] = self.Bias_array[cin_cnt] + self.InputX1_array[i][cin_cnt] * \
                                                  self.InputX2_array[i][cin_cnt]
                        if self.result[i][cin_cnt] >= 0x7fffffff:
                            self.result[i][cin_cnt] = 0x7fffffff
                        elif self.result[i][cin_cnt] <= -0x80000000:
    ...
    def cal_para(self):
        ...
        if self.tensor_en:
            ...
            self.Addr_InA_L5_step = 2 * (self.stride_y - 1) * self.Px * (self.L3_num + 1) + 2 * (self.Px - 1 - (self.Ox - 1) * self.stride_x) * (self.L3_num + 1) + 2
        else:
            self.L4_num = 0
    ...
    def save_para(self, path: str):
        with open(path, 'w') as f:
            ...
            f.write('InB_type = ' + str(self.InA_type) + '\n')
            ...
            f.write('PI_A.Addr_InA_end = ' + str(
                hex(((self.Addr_InA_base + self.Read_X1_length) >> 2) - 1) + '\n'))
            ...
            f.write('Read_X1_length = ' + str(self.Read_X1_length) + '\n')
            f.write('Read_X2_length = ' + str(self.Read_X2_length) + '\n')
