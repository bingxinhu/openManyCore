...
def gen_2M4C_map_config(phase_en, clock_in_phase, size_x, size_y, delay_l4=None, delay_l5=None,
    ...
    if phase_en[offset + 0]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            axon = p04(cin=16, cout=1024, addr_ina=0x0000 >> 2, addr_inb=0x10000 >> 2, addr_bias=0x14000 >> 2,
    ...
    if phase_en[offset + 1]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            axon = p04(cin=1024, cout=32, addr_ina=0x0020 >> 2, addr_inb=0x15000 >> 2, addr_bias=0x1D000 >> 2,
                       addr_out=0x0420 >> 2, ina_type=1, inb_type=1, load_bias=2,
            ...
            axon = p41(px=1, py=1, cin=axon.cin, cout=axon.cout, kx=1, ky=1, sx=1, sy=1,
                       ...
                       data_x=None, data_w=[], data_b=[])
            soma1 = p06(addr_in=0x0420 >> 2, addr_out=0x24000 >> 2,
            ...
            router = p09(rhead_mode=1, send_en=False if core_x % 2 == 0 else True,
                         receive_en=True if core_x % 2 == 0 else False, send_num=4 * 32 // 8 - 1,
                         receive_num=0,
            ...
            if core_x % 2 != 0:
                router.addRHead(S=0, T=1, P=0, Q=0, X=-1, Y=0,
                                A=0, pack_per_Rhead=4 * 32 // 8 - 1,
                                A_offset=0, Const=0, EN=1)
                soma2 = None
            if core_x % 2 == 0:
                soma1 = None
                soma2 = p06(addr_in=0x21000 >> 2, addr_out=0x04A0 >> 2,
        ...
        for core_y, core_x in product(range(size_y), range(size_x)):
            if core_x == 0 and core_y == 0:
                axon = p02(avg_pooling_en=False, ina_type=0, load_bias=0, kx=1, ky=2, sx=1, sy=1, cin=32, px=1, py=1,
                           addr_in=0x0420 >> 2, addr_bias=0x0, addr_out=0x8000 >> 2, pad_top=0, pad_down=0, pad_left=0,
                           pad_right=0, bias_length=0, data_x=None, data_b=None, constant_b=0)
                soma1 = pX5(mode='max', addr_in=0x8000 >> 2, addr_out=0x0520 >> 2,
...
if __name__ == '__main__':
    ...
    config = gen_2M4C_map_config(phase, clock_in_phase=clock_in_phase, size_x=4, size_y=1,
