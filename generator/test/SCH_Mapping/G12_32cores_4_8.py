...
def Gen_G12_Map_Config(phase_en, clock, M, N, start_row=0, in_data_en=0, out_data_en=0, delay_L4=(), delay_L5=()):
    """
    ResNet-50 Group12 Mapping
    ...
    if phase_en[0] == 1:
        ...
        for core in range(start_row * M, start_row * M + core_num):
            ...
            if out_data_en == 1:
                ...
                length = 10 * 32
                soma1 = pX6(core_x=core_x, core_y=core_y, type=6, length_in=length//2, length_ciso=length//2,
                            length_out=length, \
                            num_in=1, num_ciso=1, num_out=1, row_ck_on=0, addr_in=0x1c400 >> 2,
                            addr_ciso=0x1c400+25*32 >> 2,
                            addr_out=0x9000)
                s1 = soma1.init_data()
                soma1.memory_blocks = [
            ...
            if out_data_en == 1:
                ...
                if core_y == 0:
                    ...
                    dst_y = -1
                A = core_x * 4
                router.addRHead(S=0, T=1, P=0, Q=1, X=dst_x - core_x, Y=dst_y - core_y - 1, A=A,
                                    pack_per_Rhead=(router.Send_number + 1) // 2 - 1, A_offset=28, Const=3, EN=1)
    ...
    if phase_en[1] == 1:
        ...
        for core in range(start_row * M, start_row * M + core_num):
            ...
            if out_data_en == 1:
                ...
                soma1 = pX6(core_x=core_x, core_y=core_y, type=6, length_in=length // 2, length_ciso=length // 2,
                            length_out=length, \
                            num_in=1, num_ciso=1, num_out=1, row_ck_on=0, addr_in=0x1c400+5*32 >> 2,
                            addr_ciso=0x1c400 + 30 * 32 >> 2,
                            addr_out=0x9000)
                s1 = soma1.init_data()
                soma1.memory_blocks = [
                    {'name': "core_{:d}_{:d}_phase_1_soma1_in".format(core_x, core_y),
    ...
    if phase_en[3] == 1:
        ...
        for core in range(start_row * M, start_row * M + core_num):
            ...
            if out_data_en == 1:
                ...
                soma1 = pX6(core_x=core_x, core_y=core_y, type=6, length_in=length // 2, length_ciso=length // 2,
                            length_out=length, \
                            num_in=1, num_ciso=1, num_out=1, row_ck_on=0, addr_in=0x1c400 + 15 * 32 >> 2,
                            addr_ciso=0x1c400 + 40 * 32 >> 2,
                            addr_out=0x9000)
                s1 = soma1.init_data()
                soma1.memory_blocks = [
                    {'name': "core_{:d}_{:d}_phase_1_soma1_in".format(core_x, core_y),
...
#
# map_config = Gen_G12_Map_Config(phase_en, 28000, M=8 , N=4)
#
