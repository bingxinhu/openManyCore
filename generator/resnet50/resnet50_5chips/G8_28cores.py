...
def gen_g8_map_config(phase_en, clock_in_phase, size_x, size_y, chip, cuts, static_data=None,
    ...
    if phase_en[0]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            if (core_x, core_y) != (0, 0):
                axon = None
            if out_data_en and (core_x in [0, 2, 6, 8, 10]):
                if core_x == 0:
                    addr_in, num = 0x1c800 >> 2, 14
                elif core_x == 2:
                    addr_in, num = 0x1c800 >> 2, 10
                elif core_x == 6:
                    addr_in, num = 0x1d600 >> 2, 7
                elif core_x == 8:
                    addr_in, num = 0x1c800 >> 2, 14
                elif core_x == 10:
                    addr_in, num = 0x1c800 >> 2, 3
                else:
                    raise ValueError
                soma1 = p06(addr_in=addr_in, addr_out=0x24000 >> 2, addr_ciso=0x0000 >> 2, length_in=512,
                            num_in=num, length_ciso=1, num_ciso=num, length_out=512, num_out=num, type_in=1, type_out=1,
                            data_in=static_data['g8_add_output1'][(core_x, core_y)] if init_data else None)
            else:
                soma1 = None
                num = 0
            addr_rhead_base = MapConfigGen.get_router_rhead_base(phase_group[(chip, (core_x, core_y))]['prims'])
            router = p09(rhead_mode=1, send_en=out_data_en and (core_x in [0, 2, 6, 8, 10]),
                         receive_en=in_data_en, send_num=512 * num // 8 - 1, receive_num=0,
                         ...
                         data_in=None)
            if out_data_en and (core_x in [0, 2, 6, 8, 10]):
                dst_x, dst_y = (core_x // 6) + core_y * 2 + 8, 2
                if core_x == 0:
                    router.addRHead(S=0, T=1, P=0, Q=0, X=dst_x - core_x, Y=dst_y - core_y, A=0,
                                    pack_per_Rhead=512 * num // 8 - 1, A_offset=0, Const=0, EN=1)
                elif core_x == 2:
                    router.addRHead(S=0, T=1, P=0, Q=0, X=dst_x - core_x, Y=dst_y - core_y, A=14 * 512 // 8,
                                    pack_per_Rhead=512 * num // 8 - 1, A_offset=0, Const=0, EN=1)
                elif core_x == 6:
                    router.addRHead(S=0, T=1, P=0, Q=0, X=dst_x - core_x, Y=dst_y - core_y, A=0,
                                    pack_per_Rhead=512 * num // 8 - 1, A_offset=0, Const=0, EN=1)
                elif core_x == 8:
                    router.addRHead(S=0, T=1, P=0, Q=0, X=dst_x - core_x, Y=dst_y - core_y, A=7 * 512 // 8,
                                    pack_per_Rhead=512 * num // 8 - 1, A_offset=0, Const=0, EN=1)
                elif core_x == 10:
                    router.addRHead(S=0, T=1, P=0, Q=0, X=dst_x - core_x, Y=dst_y - core_y, A=21 * 512 // 8,
                                    pack_per_Rhead=512 * num // 8 - 1, A_offset=0, Const=0, EN=1)
                else:
                    raise ValueError
            if in_data_en:
    ...
    if phase_en[1]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            axon = None
            if out_data_en and (core_x in [2, 4, 6, 10, 12]):
                if core_x == 2:
                    addr_in, num = (0x1c800 + 10 * 512) >> 2, 4
                elif core_x == 4:
                    addr_in, num = 0x1c800 >> 2, 14
                elif core_x == 6:
                    addr_in, num = 0x1c800 >> 2, 7
                elif core_x == 10:
                    addr_in, num = (0x1c800 + 3 * 512) >> 2, 11
                elif core_x == 12:
                    addr_in, num = 0x1c800 >> 2, 14
                else:
                    raise ValueError
                soma1 = p06(addr_in=addr_in, addr_out=0x24000 >> 2, addr_ciso=0x0000 >> 2, length_in=512,
                            num_in=num, length_ciso=1, num_ciso=num, length_out=512, num_out=num, type_in=1, type_out=1,
                            data_in=static_data['g8_add_output2'][(core_x, core_y)] if init_data else None)
            else:
                soma1 = None
                num = 0
            addr_rhead_base = MapConfigGen.get_router_rhead_base(phase_group[(chip, (core_x, core_y))]['prims'])
            router = p09(rhead_mode=1, send_en=out_data_en and (core_x in [2, 4, 6, 10, 12]), receive_en=in_data_en,
                         send_num=512 * num // 8 - 1, receive_num=0,
                         addr_din_base=0x1000 >> 2, addr_din_length=895, addr_rhead_base=addr_rhead_base,
                         addr_rhead_length=1, addr_dout_base=0x4000 >> 2, addr_dout_length=0, soma_in_en=1,
                         data_in=None)
            if out_data_en and (core_x in [2, 4, 6, 10, 12]):
                dst_x, dst_y = (core_x // 7) + core_y * 2 + 8, 2
                if core_x == 2:
                    router.addRHead(S=0, T=1, P=0, Q=0, X=dst_x - core_x, Y=dst_y - core_y, A=0,
                                    pack_per_Rhead=512 * num // 8 - 1, A_offset=0, Const=0, EN=1)
                elif core_x == 4:
                    router.addRHead(S=0, T=1, P=0, Q=0, X=dst_x - core_x, Y=dst_y - core_y, A=4 * 512 // 8,
                                    pack_per_Rhead=512 * num // 8 - 1, A_offset=0, Const=0, EN=1)
                elif core_x == 6:
                    router.addRHead(S=0, T=1, P=0, Q=0, X=dst_x - core_x, Y=dst_y - core_y, A=18 * 512 // 8,
                                    pack_per_Rhead=512 * num // 8 - 1, A_offset=0, Const=0, EN=1)
                elif core_x == 10:
                    router.addRHead(S=0, T=1, P=0, Q=0, X=dst_x - core_x, Y=dst_y - core_y, A=0,
                                    pack_per_Rhead=512 * num // 8 - 1, A_offset=0, Const=0, EN=1)
                elif core_x == 12:
                    router.addRHead(S=0, T=1, P=0, Q=0, X=dst_x - core_x, Y=dst_y - core_y, A=11 * 512 // 8,
                                    pack_per_Rhead=512 * num // 8 - 1, A_offset=0, Const=0, EN=1)
                else:
                    raise ValueError
            if in_data_en:
    ...
    if phase_en[offset + 0]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            if static_data is None:
                ...
                data_x = None if in_data_en else static_data['conv3d1_input'][(core_x, core_y)]
                data_w = static_data['conv3d1_weight'][(core_x, core_y)]
                data_b = static_data['conv3d1_bias'][(core_x, core_y)]
            ...
            if core_x < 7 and core_y == 0:
                soma1 = pX5(mode='max', addr_in=0x1C800 >> 2, addr_out=0x8400 >> 2, cin=32, cout=32, px=28, py=1, kx=1,
                            ky=1, sx=1, sy=1, cmp_c=0, type_in=0, type_out=1, in_cut_start=cuts['layer2.3.cut1'],
    ...
    if phase_en[offset + 17]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            if core_x % 7 == 0:
                ...
                py = 6
                addr_ina = 0x9580 >> 2
            data_w = static_data['conv3d2_weight'][(core_x, core_y)]
            data_b = static_data['conv3d2_bias'][(core_x, core_y)]
            ...
            soma1 = pX5(mode='max', addr_in=0x1C800 >> 2, addr_out=0x1E400 >> 2, cin=32, cout=32, px=28, py=4, kx=1,
                        ky=1, sx=1, sy=1, cmp_c=0, type_in=0, type_out=1, in_cut_start=cuts['layer2.3.cut2'],
    ...
    if phase_en[offset + 20]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            data_w = static_data['conv3d3_weight'][(core_x, core_y)]
            data_b = static_data['conv3d3_bias'][(core_x, core_y)]
            ...
            soma1 = pX5(mode='max', addr_in=0x8400 >> 2, addr_out=0xBC00 >> 2, cin=128, cout=128, px=28, py=4, kx=1,
                        ky=1, sx=1, sy=1, cmp_c=0x80808080, type_in=0, type_out=1, in_cut_start=cuts['layer2.3.cut3'],
                        row_ck_on=1, in_row_max=1)
            router = None
            soma2 = None
            phase_group[(chip, (core_x, core_y))]['prims'].append({'axon': axon, 'soma1': soma1, 'router': router,
    ...
    if phase_en[offset + 23]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            soma1 = pX5(mode='max', addr_in=0x8400 >> 2, addr_out=0x1c800 >> 2, cin=512, cout=512, px=1, py=28,
                        kx=1, ky=1, sx=2, sy=2, cmp_c=0, type_in=0, type_out=1, in_cut_start=cuts['layer2.3.cut5'],
                        row_ck_on=1, in_row_max=2)
            router = None
            soma2 = None
            phase_group[(chip, (core_x, core_y))]['prims'].append({'axon': axon, 'soma1': soma1, 'router': router,
...
if __name__ == '__main__':
    ...
    static_data = generate_g8_data(handler, size_y=2, size_x=14)
    config = gen_g8_map_config(phase, clock_in_phase=100_000, size_x=14, size_y=2, chip=(0, 0), cuts=cuts,
