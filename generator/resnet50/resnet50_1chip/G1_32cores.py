...
def gen_g1_map_config(phase_en, clock_in_phase, size_x, size_y, cuts, static_data=None,
        ...
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            if (core_x, core_y) == (15, 1):
                if g17_en or g1_ib_en:
                    ...
                    if g1_ib_en:
                        recv_num += 3
                    recv_num -= 1
                addr_din_length = (h_in * 128 * 3 + 256) // 8 - 1
            else:
                if g1_ib_en:
                    recv_num += 2
                addr_din_length = (h_in * 128 * 3) // 8 - 1
            addr_rhead_base = MapConfigGen.get_router_rhead_base(phase_group[(chip, (core_x, core_y))]['prims'])
            router = p09(rhead_mode=1, send_en=g0_en or g2_en, receive_en=g17_en or g1_ib_en,
                         send_num=length_in - 1, receive_num=recv_num,
            ...
            if g0_en or g2_en:
                ...
                if core_x == 0 and core_y == 0:
                    router.addRHead(S=0, T=1, P=0, Q=0, X=7 - core_x, Y=-1 - core_y, A=0,
                                    pack_per_Rhead=32 // 8 - 1, A_offset=0, Const=0, EN=g0_en)
            if not (g17_en or g1_ib_en):
                soma2 = None
            else:
    ...
    if phase_en[1]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            addr_rhead_base = MapConfigGen.get_router_rhead_base(phase_group[(chip, (core_x, core_y))]['prims'])
            router = p09(rhead_mode=1, send_en=send_en, receive_en=recv_en, send_num=256 // 8 - 1, receive_num=0,
                         addr_din_base=0x980, addr_din_length=256 // 8 - 1, addr_rhead_base=addr_rhead_base,
                         addr_rhead_length=0, addr_dout_base=0x980, addr_dout_length=256 // 16 - 1, soma_in_en=0,
            ...
            if g17_en:
                if core_y == 1:
                    if core_x == 0:
                        ...
                        router.addRHead(S=0, T=1, P=0, Q=1, X=-1, Y=0, A=0, pack_per_Rhead=256 // 8 - 1,
                    ...
                    if core_x == 15:
                        ...
                        router.CXY, router.Nx, router.Ny = 1, 1, 0
            if g17_en:
                soma2 = p06(addr_in=0x8980, addr_out=0x0000 >> 2, addr_ciso=0x10000 >> 2, length_in=256,
                            num_in=1, length_ciso=1, num_ciso=1, length_out=256, num_out=1,
                            type_in=1, type_out=1)
            else:
                soma2 = None
            phase_group[(chip, (core_x, core_y))]['prims'].append({'axon': axon, 'soma1': soma1, 'router': router,
