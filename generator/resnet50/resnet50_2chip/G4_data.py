...
def generate_g4_data(handler, size_y, size_x):
    ...
    y_output_raw_data = handler.parameters['layer1.2.cut5']['output'].astype(np.int8)
    y_avg_pool_raw_data = handler.parameters['avgpool']['output'].astype(np.int32)
    y_avg_pool_cut_raw_data = handler.parameters['avgpool_cut']['output'].astype(np.int8)
    ...
    for core_y, core_x in product(range(size_y), range(size_x)):
        ...
        if core_x == 0 and core_y == 0:
            out_dict[(core_x, core_y)] = ((0, 256), (0, 1), (0, 1))
        else:
            out_dict[(core_x, core_y)] = ((0, 256), (0, 0), (0, 0))
    ...
    data['avgpool']['output'] = ResNetDataHandler.tensor_split(
        raw_data=y_avg_pool_raw_data, split_dict=out_dict, data_type=0, alignment=(16, None, None),
        dims=(0, 2, 1))
    data['avgpool_cut']['output'] = ResNetDataHandler.tensor_split(
        raw_data=y_avg_pool_cut_raw_data, split_dict=out_dict, data_type=1, alignment=(16, None, None),
        dims=(0, 2, 1))
