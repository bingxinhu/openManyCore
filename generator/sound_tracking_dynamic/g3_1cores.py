...
def gen_3_map_config(phase_en, clock_in_phase, size_x, size_y, handler, data=None, delay_l4=None, delay_l5=None,
                     in_data_en=False, chip=(0, 0), out_data_en=False, init_data=False, send_to_fpga=False):
    """
    ...
    if phase_en[0]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            soma1 = None
            if out_data_en:
                soma1 = p06(addr_in=0x1000 >> 2, addr_out=0x9000, addr_ciso=0x0000 >> 2, length_in=16,
                            length_out=16, length_ciso=1, num_in=1, num_ciso=1, num_out=1, type_in=1,
                            type_out=1, in_cut_start=0, in_row_max=0, row_ck_on=0,
                            data_in=data['g3_output'][(core_x, core_y)] if init_data else None,
                            data_ciso=None)
            router = p09(rhead_mode=1, send_en=out_data_en,
                         receive_en=in_data_en, send_num=1, receive_num=0,
                         addr_din_base=0x380, addr_din_length=880 // 8 - 1, addr_rhead_base=0x300,
                         addr_rhead_length=0, addr_dout_base=0x1000, addr_dout_length=0, soma_in_en=1,
                         cxy=0, relay_num=0, nx=0, ny=0, data_in=None)
            if out_data_en:
                if send_to_fpga:
                    router.addRHead(S=0, T=1, P=0, Q=0, X=0, Y=-5, A=0, pack_per_Rhead=1, A_offset=0, Const=0, EN=1)
                else:
                    router.addRHead(S=0, T=1, P=0, Q=0, X=5, Y=-3, A=0, pack_per_Rhead=1, A_offset=0, Const=0, EN=1)
            soma2 = p06(addr_in=0x8380 + ((39 * 16) >> 2), addr_out=0x0000 >> 2, addr_ciso=0x10000 >> 2, length_in=256,
                        length_out=256, length_ciso=1, num_in=1, num_ciso=1, num_out=1, type_in=1,
                        type_out=1, in_cut_start=0, in_row_max=0, row_ck_on=0,
    ...
    if phase_en[offset + 0]:
        for core_y, core_x in product(range(size_y), range(size_x)):
            ...
            axon = p04(cin=256, cout=2, addr_ina=0x0000 >> 2, addr_inb=0x10000 >> 2, addr_bias=0x12000 >> 2,
                       addr_out=0x12080 >> 2, ina_type=1, inb_type=1, load_bias=2,
            ...
            soma1 = None
            router = None
            soma2 = p07(addr_in=0x12080 >> 2, addr_out=0x1000 >> 2, addr_lut=0x12100 >> 2,
