...
def gen_g8_ob_map_config(phase_en, clock_in_phase, data=None, chip=(0, 1),
                         in_data_en=False, out_data_en=False, delay_l4=None, delay_l5=None, init_data=None):
    """
        ResNet-50 5-Chip Group-8 Output Buffer
        core array : chip(0, 1)   [(8, 9), (9, 9), (10, 9), (11, 9)]
    """
    map_config = {
        'sim_clock': None,
        (chip, 0): {
        ...
        for core_x, core_y in [(8, 9), (9, 9), (10, 9), (11, 9)]:
            ...
            if out_data_en:
                soma1 = pX5(mode='max', addr_in=0x19000 >> 2, addr_out=0x9000, cin=512, cout=512, px=4, py=6, kx=1,
                            ...
                            in_row_max=1, data_in=data['layer3.0.conv1']['input1'][(core_x, 0)] if init_data else None)
            if core_x in [8, 10]:
                recv_num = 1
            else:
                recv_num = 2
            addr_rhead_base = MapConfigGen.get_router_rhead_base(phase_group_0[(chip, (core_x, core_y))]['prims'],
                                                                 limit=0x380)
            router = p09(rhead_mode=1, send_en=out_data_en,
                         receive_en=in_data_en, send_num=24*512//8-1, receive_num=recv_num,
        ...
        for core_x, core_y in [(8, 9), (9, 9), (10, 9), (11, 9)]:
            ...
            if out_data_en:
                soma1 = pX5(mode='max', addr_in=0x1c000 >> 2, addr_out=0x9000, cin=512, cout=512, px=5, py=5, kx=1,
                            ky=1, sx=1, sy=1, cmp_c=0x80808080, type_in=1, type_out=1, in_cut_start=0, row_ck_on=0,
                            in_row_max=1, data_in=data['layer3.0.conv1']['input2'][(core_x, 0)] if init_data else None)
            if core_x in [8, 10]:
                recv_num = 2
            else:
                recv_num = 1
            addr_rhead_base = MapConfigGen.get_router_rhead_base(phase_group_0[(chip, (core_x, core_y))]['prims'],
                                                                 limit=0x380)
            router = p09(rhead_mode=1, send_en=out_data_en,
                         receive_en=in_data_en, send_num=25*512//8-1, receive_num=recv_num,
